                          ...
                          long unsigned int border_color, char *name, XFontStruct *font)
{
    ...
    new->window = XCreateSimpleWindow(display, parent, x, y, new->width, new->height, 0, border_color, WHITE(display));
    XSetWindowColormap(display, new->window, ColorMap);
    /*
     * Now create the tools needed
     */
    B = XfCreateButton(display, new->window, 15, 15, 15, 390, border_width, border_color, "", 1);
    B->member = (int *)new;
    XfActivateButton(B, ExposureMask);
    new->letters = B;

    sprintf(newname, "%s fnames", name);
    B = XfCreateButton(display, new->window, 40, 15, 220, 390, border_width, border_color, newname, 1);
    B->member = (int *)new;
    XfActivateButton(B, ExposureMask);
    new->fnames = B;

    sprintf(newname, "%s bands", name);
    B = XfCreateButton(display, new->window, 265, 15, 25, 390, border_width, border_color, newname, 1);
    B->member = (int *)new;
    XfAddButtonCallback(B, 0, XF_CALLBACK(load_band), NULL);
    ...
    for (i = 0; i < NIMAGE; i++) {
        ...
        B = XfCreateButton(display, new->window, 300, new_y + 15, 15, 15, border_width, border_color, newname, 5);
        B->member = (int *)new;
        if (i == 0) {
            button_vis_1 = XfCreateVisual(B, 0, 0, 0, 0, WHITE(display), WHITE(display), XfSolidVisual);
            button_vis_2 = XfCreateVisual(B, 0, 0, 0, 0, pwRed.pixel, WHITE(display), XfSolidVisual);
            button_vis_3 = XfCreateVisual(B, 0, 0, 0, 0, pwGreen.pixel, WHITE(display), XfSolidVisual);
            button_vis_4 = XfCreateVisual(B, 0, 0, 0, 0, pwBlue.pixel, WHITE(display), XfSolidVisual);
            button_vis_5 = XfCreateVisual(B, 0, 0, 0, 0, pwYellow.pixel, WHITE(display), XfSolidVisual);
            button_vis_6 = XfCreateVisual(B, 0, 0, 0, 0, pwBackground.pixel, WHITE(display), XfSolidVisual);
        }
        B->ext = PW_CAST_INT(i);
        ...
        XfAddButtonCallback(B, 0, XF_CALLBACK(RGB_pushbutton), NULL);
        XfAddButtonCallback(B, 1, XF_CALLBACK(RGB_pushbutton), NULL);
        XfAddButtonCallback(B, 2, XF_CALLBACK(RGB_pushbutton), NULL);
        XfAddButtonCallback(B, 3, XF_CALLBACK(RGB_pushbutton), NULL);
        XfActivateButton(B, (ExposureMask | ButtonPressMask));
        new->rgbs[i] = B;

        sprintf(newname, "%s COMP %d", name, i);
        B = XfCreateButton(display, new->window, 315, new_y + 15, 15, 15, border_width, border_color, newname, 3);
        if (B == NULL)
            return (NULL);
        B->member = (int *)new;
        B->ext = PW_CAST_INT(i);
        ...
        XfAddButtonCallback(B, 0, XF_CALLBACK(overlay_pushbutton), NULL);
        XfAddButtonCallback(B, 1, XF_CALLBACK(overlay_pushbutton), NULL);
        XfActivateButton(B, (ExposureMask | ButtonPressMask));
        new->comps[i] = B;

        sprintf(newname, "%c", ('A' + (char)i));
        B = XfCreateButton(display, new->letters->window, 0, 15 * i, 15, 15, 0, WHITE(display), newname, 3);
        new->letter[i] = B;

        vis = XfCreateVisual(new->letter[i], 0, 3, 15, 15, BLACK(display), WHITE(display), XfTextVisual, newname, font,
                             0);
        XfAddButtonVisual(new->letter[i], 0, vis);
        vis = XfCreateVisual(new->letter[i], 0, 3, 15, 15, BLACK(display), pwHilite.pixel, XfTextVisual, newname, font,
                             0);
        XfAddButtonVisual(new->letter[i], 1, vis);
        vis = XfCreateVisual(new->letter[i], 0, 3, 15, 15, BLACK(display), pwBackground.pixel, XfTextVisual, newname,
                             font, 0);
        XfAddButtonVisual(new->letter[i], 2, vis);

        XfAddButtonCallback(new->letter[i], 1, XF_CALLBACK(LoadImage), NULL);
        new->letter[i]->member = (int *)new;
        new->letter[i]->ext = PW_CAST_INT(i);
        XfActivateButton(B, ExposureMask | ButtonPressMask);
        ...
        vis = XfCreateVisual(new->bands, 0, (new_y + 3), 25, 15, BLACK(display), WHITE(display), XfTextVisual, newname,
                             font, 0);
        XfAddButtonVisual(new->bands, 0, vis);
        new->bnd[i] = vis;

        sprintf(newname, "%s FNAME %d", name, i);
        B = XfCreateButton(display, new->fnames->window, 0, new_y, 220, 15, 0, border_color, newname, 3);
        if (B == NULL)
            return (NULL);
        B->member = (int *)new;
        B->ext = PW_CAST_INT(i);
        ...
        vis = XfCreateVisual(B, 0, 3, 0, 0, BLACK(display), WHITE(display), XfTextVisual, newname, font, 1);
        XfAddButtonVisual(B, 0, vis);
        XfAddButtonCallback(B, 0, XF_CALLBACK(load_filename), NULL);
    ...
    B = XfCreateButton(display, new->window, x, y, width, height, 1, BLACK(display), "UserMsg", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 3, 0, 0, BLACK(display), WHITE(display), XfTextVisual, "", font, 1));
    XfActivateButton(B, ExposureMask);
    new->UserMsg = B;
    UserMsg = B;
    B->member = (int *)new;
    ...
    B = XfCreateButton(display, new->window, x, y, width, height, 1, BLACK(display), "DeleteImage", 1);
    XfAddButtonVisual(B, 0,
                      XfCreateVisual(B, 0, 3, 0, 0, BLACK(display), WHITE(display), XfTextVisual, "DEL", font, 0));
    XfAddButtonCallback(B, 0, XF_CALLBACK(DeleteImage), NULL);
    XfActivateButton(B, ExposureMask | ButtonPressMask);
    new->DeleteImage = B;
    B->member = (int *)new;

    x += width + 5;
    B = XfCreateButton(display, new->window, x, y, width * 2 + 5, height, 1, BLACK(display), "ReadRestart", 1);
    XfAddButtonVisual(B, 0,
                      XfCreateVisual(B, 0, 3, 0, 0, BLACK(display), WHITE(display), XfTextVisual, "RESTART", font, 0));
    XfAddButtonCallback(B, 0, XF_CALLBACK(ActivateRestartRequestor), NULL);
    XfActivateButton(B, ExposureMask | ButtonPressMask);
    new->RestartRead = B;
    B->member = (int *)new;
    ...
    B = XfCreateButton(display, new->window, 145, y, 185, 70, 1, BLACK(display), "bg", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 0, 0, 0, pwBackground.pixel, pwBackground.pixel, XfSolidVisual));
    XfActivateButton(B, ExposureMask);
    bg = B;
    ...
    MakeTextButtonBg(display, bg->window, x - width - 5, y, width, height, pwBackground.pixel, font, "Header", 0);

    MakeTextButtonBg(display, bg->window, x, y - 10, width, height, pwBackground.pixel, font, "Format", 0);
    new->HeaderFormat = XfCreateButton(display, bg->window, x, y, width, height, 1, BLACK(display), "Format", 1);
    x += width + 5;
    MakeTextButtonBg(display, bg->window, x, y - 10, width + 4, height, pwBackground.pixel, font, "Org", 0);
    new->HeaderOrg = XfCreateButton(display, bg->window, x, y, width, height, 1, BLACK(display), "Org", 1);
    x += width + 5;
    MakeTextButtonBg(display, bg->window, x, y - 10, width, height, pwBackground.pixel, font, "Label", 0);
    new->HeaderLabel = XfCreateButton(display, bg->window, x, y, width, height, 1, BLACK(display), "Label", 1);

    x = 50 - width - 5;
    y += height * 2 + 5;

    MakeTextButtonBg(display, bg->window, x, y - 10, width, height, pwBackground.pixel, font, "Lines", 0);
    new->HeaderLines = XfCreateButton(display, bg->window, x, y, width, height, 1, BLACK(display), "Lines", 1);
    x += width + 5;
    MakeTextButtonBg(display, bg->window, x, y - 10, width, height, pwBackground.pixel, font, "Samples", 0);
    new->HeaderSamples = XfCreateButton(display, bg->window, x, y, width, height, 1, BLACK(display), "Samples", 1);
    x += width + 5;
    MakeTextButtonBg(display, bg->window, x, y - 10, width, height, pwBackground.pixel, font, "Bands", 0);
    new->HeaderBands = XfCreateButton(display, bg->window, x, y, width, height, 1, BLACK(display), "Bands", 1);
    x += width + 5;
    MakeTextButtonBg(display, bg->window, x, y - 10, width, height, pwBackground.pixel, font, "Order", 0);
    new->HeaderOrder = XfCreateButton(display, bg->window, x, y, width, height, 1, BLACK(display), "Order", 1);

    AddTextVisual(new->HeaderSamples, 0, BLACK(display), WHITE(display), "--", font, 0);
    AddTextVisual(new->HeaderLines, 0, BLACK(display), WHITE(display), "--", font, 0);
    AddTextVisual(new->HeaderBands, 0, BLACK(display), WHITE(display), "--", font, 0);
    AddTextVisual(new->HeaderFormat, 0, BLACK(display), WHITE(display), "--", font, 0);
    AddTextVisual(new->HeaderOrg, 0, BLACK(display), WHITE(display), "--", font, 0);
    AddTextVisual(new->HeaderLabel, 0, BLACK(display), WHITE(display), "--", font, 0);
    AddTextVisual(new->HeaderOrder, 0, BLACK(display), WHITE(display), "--", font, 0);

    XfAddButtonCallback(new->HeaderSamples, 0, XF_CALLBACK(GetInput_Samples), NULL);
    XfAddButtonCallback(new->HeaderLines, 0, XF_CALLBACK(GetInput_Lines), NULL);
    XfAddButtonCallback(new->HeaderBands, 0, XF_CALLBACK(GetInput_Bands), NULL);
    XfAddButtonCallback(new->HeaderFormat, 0, XF_CALLBACK(GetInput_Format), NULL);
    XfAddButtonCallback(new->HeaderOrg, 0, XF_CALLBACK(GetInput_Org), NULL);
    XfAddButtonCallback(new->HeaderLabel, 0, XF_CALLBACK(GetInput_Label), NULL);
    XfAddButtonCallback(new->HeaderOrder, 0, XF_CALLBACK(GetInput_Order), NULL);
    ...
    B = XfCreateButton(display, new->window, 15, 510, 315, 45, 1, BLACK(display), "bg", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 0, 0, 0, pwBackground.pixel, pwBackground.pixel, XfSolidVisual));
    XfActivateButton(B, ExposureMask);
    bg = B;
    ...
    MakeTextButtonBg(display, bg->window, x, y + 3, width, height, pwBackground.pixel, font, "Subset", 0);
    x += width + 3;
    MakeTextButtonBg(display, bg->window, x, y - 10, width, height, pwBackground.pixel, font, "Start L", 0);
    new->SubsetLine = XfCreateButton(display, bg->window, x, y, width, height, 1, BLACK(display), "StartLine", 1);
    x += width + 5;
    MakeTextButtonBg(display, bg->window, x, y - 10, width + 3, height, pwBackground.pixel, font, "Start S", 0);
    new->SubsetSample = XfCreateButton(display, bg->window, x, y, width, height, 1, BLACK(display), "StartSample", 1);
    x += width + 5;
    MakeTextButtonBg(display, bg->window, x, y - 10, width, height, pwBackground.pixel, font, "Height", 0);
    new->SubsetHeight = XfCreateButton(display, bg->window, x, y, width, height, 1, BLACK(display), "Height", 1);
    x += width + 5;
    MakeTextButtonBg(display, bg->window, x, y - 10, width, height, pwBackground.pixel, font, "Width", 0);
    new->SubsetWidth = XfCreateButton(display, bg->window, x, y, width, height, 1, BLACK(display), "Width", 1);
    x += width + 5;
    MakeTextButtonBg(display, bg->window, x, y - 10, width, height, pwBackground.pixel, font, "L Skip", 0);
    new->SubsetLSkip = XfCreateButton(display, bg->window, x, y, width, height, 1, BLACK(display), "LineSkip", 1);
    x += width + 5;
    MakeTextButtonBg(display, bg->window, x, y - 10, width, height, pwBackground.pixel, font, "S Skip", 0);
    new->SubsetSSkip = XfCreateButton(display, bg->window, x, y, width, height, 1, BLACK(display), "SampleSkip", 1);

    AddTextVisual(new->SubsetLine, 0, BLACK(display), WHITE(display), "--", font, 0);
    AddTextVisual(new->SubsetSample, 0, BLACK(display), WHITE(display), "--", font, 0);
    AddTextVisual(new->SubsetWidth, 0, BLACK(display), WHITE(display), "--", font, 0);
    AddTextVisual(new->SubsetHeight, 0, BLACK(display), WHITE(display), "--", font, 0);
    AddTextVisual(new->SubsetLSkip, 0, BLACK(display), WHITE(display), "--", font, 0);
    AddTextVisual(new->SubsetSSkip, 0, BLACK(display), WHITE(display), "--", font, 0);

    XfAddButtonCallback(new->SubsetLine, 0, XF_CALLBACK(GetInput_SubsetLine), NULL);
    XfAddButtonCallback(new->SubsetSample, 0, XF_CALLBACK(GetInput_SubsetSample), NULL);
    XfAddButtonCallback(new->SubsetWidth, 0, XF_CALLBACK(GetInput_SubsetWidth), NULL);
    XfAddButtonCallback(new->SubsetHeight, 0, XF_CALLBACK(GetInput_SubsetHeight), NULL);
    XfAddButtonCallback(new->SubsetLSkip, 0, XF_CALLBACK(GetInput_SubsetLSkip), NULL);
    XfAddButtonCallback(new->SubsetSSkip, 0, XF_CALLBACK(GetInput_SubsetSSkip), NULL);
    ...
    B = XfCreateButton(display, new->window, x, y, width, height, 1, BLACK(display), "SlowMem", 2);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 3, 0, 0, BLACK(display), WHITE(display), XfTextVisual, "S", font, 0));
    XfAddButtonVisual(B, 1,
                      XfCreateVisual(B, 0, 3, 0, 0, BLACK(display), pwBackground.pixel, XfTextVisual, "S", font, 0));
    XfAddButtonCallback(B, 0, XF_CALLBACK(toggle_mem), NULL);
    XfAddButtonCallback(B, 1, XF_CALLBACK(toggle_mem), NULL);
    XfActivateButton(B, ButtonPressMask | ExposureMask);
    new->SlowMem = B;
    B->member = (int *)new;
    B->ext = 0;

    B = XfCreateButton(display, new->window, x + width, y, width, height, 1, BLACK(display), "MediumMem", 2);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 3, 0, 0, BLACK(display), WHITE(display), XfTextVisual, "M", font, 0));
    XfAddButtonVisual(B, 1,
                      XfCreateVisual(B, 0, 3, 0, 0, BLACK(display), pwBackground.pixel, XfTextVisual, "M", font, 0));
    XfAddButtonCallback(B, 0, XF_CALLBACK(toggle_mem), NULL);
    XfAddButtonCallback(B, 1, XF_CALLBACK(toggle_mem), NULL);
    XfActivateButton(B, ButtonPressMask | ExposureMask);
    new->MediumMem = B;
    ...
    B = XfCreateButton(display, new->window, x + width * 2, y, width, height, 1, BLACK(display), "FastMem", 2);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 3, 0, 0, BLACK(display), WHITE(display), XfTextVisual, "F", font, 0));
    XfAddButtonVisual(B, 1,
                      XfCreateVisual(B, 0, 3, 0, 0, BLACK(display), pwBackground.pixel, XfTextVisual, "F", font, 0));
    XfAddButtonCallback(B, 0, XF_CALLBACK(toggle_mem), NULL);
    XfAddButtonCallback(B, 1, XF_CALLBACK(toggle_mem), NULL);
    XfActivateButton(B, ButtonPressMask | ExposureMask);
    new->FastMem = B;
    ...
    B = XfCreateButton(display, new->window, x, y, width, height, 1, BLACK(display), "UsedColors", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 3, 0, 0, BLACK(display), WHITE(display), XfTextVisual, "0", font, 0));
    XfAddButtonCallback(B, 0, XF_CALLBACK(load_ncolors), NULL);
    ...
    B = XfCreateButton(display, new->window, x, y, width, height, 0, BLACK(display), "AvailColors", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 3, 0, 0, BLACK(display), WHITE(display), XfTextVisual, "0", font, 0));
    XfActivateButton(B, ExposureMask);
    new->AvailColors = B;
    B->member = (int *)new;
...
int UpdateRequestor(Requestor R, int n)
{
    ...
    for (i = 0; i < NIMAGE; i++) {
        ...
        if (i != n) {
            ((R->fname[i])->States[0])->Visuals->background = WHITE(R->fname[i]->display);
        } else {
            ((R->fname[i])->States[0])->Visuals->background = pwBackground.pixel;
        }
...
void RGB_pushbutton(Button B, XEvent *E)
{
    ...
    i = PW_CAST_PTR_INT(B->ext);
    j = PW_CAST_PTR_INT(R->fnames->ext);
    ...
    BB = R->comps[PW_CAST_PTR_INT(B->ext)];
    Images[j]->overlays[i] = 0;
    BB->state = 0;

    Images[j]->rgb[PW_CAST_PTR_INT(B->ext)] = B->state;
...
void overlay_pushbutton(Button B, XEvent *E)
{
    ...
    if (Images[j] == NULL || Images[i] == NULL || (Images[i])->composite == 1) {
        XBell(B->display, 50);
        return;
    }
    ...
    B->state = 1 - B->state;
    Images[j]->overlays[PW_CAST_PTR_INT(B->ext)] = B->state;
...
void load_filename(Button B, XEvent *E)
{
    ...
    R = (Requestor)B->member;
    i = PW_CAST_PTR_INT(R->fnames->ext);
    display = B->display;
    ...
    if (PW_CAST_PTR_INT(B->ext) == i) {
         ...
         *
         */
        s = xgets(display, B->window, 0, 0, B->width, B->height, WHITE(display), BLACK(display), font, p, E);
    ...
    load_info(Images[PW_CAST_PTR_INT(R->fnames->ext)], R);
}
...
int off_load_filename(int i, char *buf, Requestor R, int stretch, int colors, int band)
{
    ...
    memcpy((char *)&(Images[i]->header), (char *)header, sizeof(struct vicar_header));
...
void load_band(Button B, XEvent *E)
{
    ...
    s = xgets(display, B->window, R->bnd[i]->x_pos, R->bnd[i]->y_pos - 3, R->bnd[i]->width, R->bnd[i]->height,
...
int free_image(Display *display, Image *new)
{
    ...
    if (CC != NULL && CC->image == old) {
        CC->image = NULL;
        CC->image_index = -1;
    }
    if (Mag != NULL && Mag->image == old)
        Mag->image = NULL;
    ...
    old->filename = NULL;
    if (old->ximage) {
        pw_free_rgb_image(old->ximage);
        old->ximage = NULL;
        old->rgb_data = NULL;
    } else if (old->rgb_data) {
        free((char *)old->rgb_data);
        old->rgb_data = NULL;
    }
    old->ximage = NULL;
    if (old->pixmap)
    ...
    old->MapPhoto = NULL;
    if (old->hist_image) {
        pw_free_rgb_image(old->hist_image);
        old->hist_image = NULL;
        old->hist_data = NULL;
    } else if (old->hist_data) {
        free((char *)old->hist_data);
        old->hist_data = NULL;
    }
    if (old->pan_image) {
        pw_free_rgb_image(old->pan_image);
        old->pan_image = NULL;
        old->pan_data = NULL;
    } else if (old->pan_data) {
        free((char *)old->pan_data);
        old->pan_data = NULL;
    }
    if (old->Colors) {
        free((char *)old->Colors);
        old->Colors = NULL;
    }
    if (old->map) {
        free((char *)old->map);
    ...
    if (old->quant) {
        free(old->quant[0].pixels);
        free(old->quant);
        old->quant = NULL;
    }
    if (old->Dranges) {
        free((char *)old->Dranges);
        old->Dranges = NULL;
    }
...
void toggle_mem(Button B, XEvent *E)
{
    ...
    R = (Requestor)B->member;
    if (Images[PW_CAST_PTR_INT(R->fnames->ext)] == NULL)
        return;
    ...
    Images[PW_CAST_PTR_INT(R->fnames->ext)]->MemMode = PW_CAST_PTR_INT(B->ext);

    UpdateButton(R->SlowMem);
    UpdateButton(R->MediumMem);
    UpdateButton(R->FastMem);

    if (load_fast_mem(B->display, Images[PW_CAST_PTR_INT(R->fnames->ext)]) == 0) {
        B->state = 0;
        R->SlowMem->state = 1;
        Images[PW_CAST_PTR_INT(R->fnames->ext)]->MemMode = 0;
        UpdateButton(R->SlowMem);
        UpdateButton(R->MediumMem);
        UpdateButton(R->FastMem);
    }
...
int load_info(Image new, Requestor R)
{
    ...
    if (new == NULL) {
        ...
        if (new->header.label_size == 0)
            sprintf(buf, "--");
        else
            sprintf(buf, "%d", new->header.label_size);
        strcpy(t, buf);
        ...
        if (new->subset.uwidth == 0)
            sprintf(buf, "--");
        else
            sprintf(buf, "%d", new->subset.uwidth);
        strcpy(t, buf);

        t = (R->SubsetHeight->States[0])->Visuals->visual.t_vis.text;
        if (new->subset.uheight == 0)
            sprintf(buf, "--");
        else
            sprintf(buf, "%d", new->subset.uheight);
        strcpy(t, buf);
...
void LoadImage(Button B, XEvent *E)
{
    ...
    if (Images[i]->band >= 0) {
        ...
        if ((Images[i]->force_reload || Images[i]->data == NULL) && Images[i]->composite == 0) {
...
int create_hist(Display *display, Image new, int size, int type, int scale)
{
    ...
    if (type)
        make_hist_dist(new, C, size, scale);
    else
        make_hist_freq(new, C, size, scale);
    for (i = 0; i < n; i++) {
        for (j = 0; j < (n - C[i]); j++)
            idata[i + j * n] = 1;
        for (j = (n - C[i]); j < n; j++)
    ...
    if (new->hist_image != NULL) {
        pw_free_rgb_image(new->hist_image);
        new->hist_image = NULL;
        new->hist_data = NULL;
    }
    {
        ...
        save2 = Colors[2];
        Colors[2].red = pwBlue.red;
        Colors[2].green = pwBlue.green;
        Colors[2].blue = pwBlue.blue;
        Colors[2].flags = DoRed | DoGreen | DoBlue;

        rgb = (unsigned char *)malloc((unsigned int)npixels * bpp);
        if (rgb != NULL) {
            pw_lut_index_to_rgb(NULL, (unsigned char *)idata, rgb, npixels);
            new->hist_data = (char *)rgb;
            new->hist_image = pw_create_rgb_image(display, rgb, n, n);
        }
        Colors[2] = save2;
    }
    free(idata);
    ...
    for (i = 0; i < ncolors; i++) {
        if (map[i] != 0) {
            for (j = 0; j < quant[i].count; j++) {
                data[quant[i].pixels[j]] = (unsigned char)(start + i + 2);
            }
        }
    }
    return 0;
    ...
    for (i = 0; i < NIMAGE; i++) {
        if (new->overlays[i]) {
            if (Images[i]->quant == NULL) {
                ...
                quantize(Images[i]->sdata, Images[i]->ncolors, Images[i]->subset.width, Images[i]->subset.height,
                         &(Images[i]->quant), 2);
...
int load_fast_mem(Display *display, Image new)
{
    ...
    if (new->MemMode == 1) {
        AllocError = 0;
        new->pixmap = XCreatePixmap(display, RootWindow(display, DefaultScreen(display)), IWidth, IHeight,
                                    DefaultDepth(display, DefaultScreen(display)));
        XSync(display, False);
        if (!AllocError) {
            XPutImage(display, new->pixmap, DefaultGC(display, DefaultScreen(display)), new->ximage, 0, 0, 0, 0, IWidth,
                      IHeight);
        } else {
            new->pixmap = (Pixmap)NULL;
            return 0;
        ...
        AllocError = 0;
        new->pixmap = XCreatePixmap(display, RootWindow(display, DefaultScreen(display)), new->subset.width,
                                    new->subset.height, DefaultDepth(display, DefaultScreen(display)));
        XSync(display, False);
        if (!AllocError) {
            XPutImage(display, new->pixmap, DefaultGC(display, DefaultScreen(display)), new->ximage, 0, 0, 0, 0,
...
int create_pan(Display *display, Image new)
{
    ...
    max = (new->subset.height > new->subset.width ? new->subset.height : new->subset.width);
    ...
    for (y = 0; y < ysize; y++) {
        for (x = 0; x < xsize; x++) {
            data[y * xsize + x] = new->sdata[((int)(y * rat) * new->subset.width + (int)(x * rat))];
        }
    }
    if (new->pan_image != NULL) {
        pw_free_rgb_image(new->pan_image);
        new->pan_image = NULL;
        new->pan_data = NULL;
    } else if (new->pan_data != NULL) {
        free(new->pan_data);
        new->pan_data = NULL;
    }
    {
        int npixels = xsize * ysize;
        unsigned char *rgb = (unsigned char *)malloc((unsigned int)npixels * 4);
        if (rgb == NULL) {
            free(data);
            return 0;
        }
        pw_lut_index_to_rgb(new, (unsigned char *)data, rgb, npixels);
        new->pan_data = (char *)rgb;
        new->pan_image = pw_create_rgb_image(display, rgb, xsize, ysize);
    }
    free(data);
    return 0;
}
...
int set_no_header(Image new)
{
    new->header.label_size = new->header.format = new->header.record_size = new->header.org = new->header.lines =
        new->header.samples = new->header.bands = 0;
    new->byte_order = 0;
    new->band = -1;
    return 0;
}
...
int compute_subset(Requestor R, int i)
{
    ...
    if (x > new->header.samples)
        x = new->header.samples;
    if (y > new->header.lines)
        y = new->header.lines;

    if (width == 0)
        width = new->header.samples - x + 1;
    if (height == 0)
        height = new->header.lines - y + 1;

    if (x + width > new->header.samples)
        width = new->header.samples - x + 1;
    if (y + height > new->header.lines)
        height = new->header.lines - y + 1;
