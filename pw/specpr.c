...
void julian_date(int *secs, int *date);
...
#define swap4byte(s)                                                                                                   \
    (swap1byte(((byteptr)(s))[0], ((byteptr)(s))[3]), swap1byte(((byteptr)(s))[1], ((byteptr)(s))[2]), (s))
void byteorder(int *a, int iflag)
{
    int itmp, i, j;
    ...
    if (iflag == 1)
        swap4byte(&((int *)a)[0]); /* swap byte order on bitflags */
...
void write_specpr(int fd, int i, struct _label *label, char *data)
{
    int type;
    int chans;
    struct _tlabel *tlabel;
    ...
    switch (check_bit(label->icflag, 1) ? 1 : 0) {
    case 0:
        /* data */
...
struct _label *make_header(char *filename, int npixels, int waves, char *title, char *ahist, char *mhist)
{
    int i;
