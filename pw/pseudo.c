...
int build_table(unsigned char *red, unsigned char *green, unsigned char *blue, struct table_entry *table,
                struct pixel_entry *pixels, int length, int n_pixels, unsigned char *red_lut, unsigned char *green_lut,
                unsigned char *blue_lut, int n_red, int n_blue, int n_green, int n_bits, char base);
...
void make_split(struct partition *node, struct table_entry *table, struct partition *a, struct partition *b);
...
int pseudocolor(unsigned char *red, unsigned char *green, unsigned char *blue, unsigned char *out, int n_red,
                int n_green, int n_blue, int n_bits, int n_out, unsigned char *red_lut, unsigned char *green_lut,
                unsigned char *blue_lut, unsigned char *out_map, char base, int size)
{
    ...
    ncolors = build_table(red, green, blue, table, pixels, length, n_pixels, red_lut, green_lut, blue_lut, n_red,
                          n_blue, n_green, n_bits, base);
...
int allocate_table(struct table_entry **table, int n_bits)
{
    ...
    *table = (struct table_entry *)calloc(sizeof(struct table_entry), (unsigned int)length);
    if (*table == NULL) {
...
int allocate_pixels(struct pixel_entry **pixels, int length)
{
    *pixels = (struct pixel_entry *)calloc(sizeof(struct pixel_entry), (unsigned int)length);
    if (*pixels == NULL) {
                ...
                unsigned char *blue_lut, int n_red, int n_blue, int n_green, int n_bits, char base)
{
    ...
    for (i = 0; i < n_pixels; i++) {
        color = *(shift_red + (*++red_ptr)) | *(shift_green + (*++green_ptr)) | *(shift_blue + (*++blue_ptr));
        t = table + color;
        t->count++;
        (++next_pixel)->pixel = i;
    ...
    for (next_entry = 0;; next_entry++) {
        ...
        if (source != next_entry) {
            memcpy((char *)(table + next_entry), (char *)(table + source), sizeof(struct table_entry));
        }
        source++;
    }
...
int merge_frequency(struct table_entry *table, int ncolors, int n_out)
{
    int square[513];
    int *square_zero;
    int i, j;
    ...
    for (index = n_out; index < ncolors; index++) {
        ...
        for (j = 0; j < n_out; j++) {
            ...
            dist = *(square_zero + red_diff) + *(square_zero + green_diff) + *(square_zero + blue_diff);
...
void max_bounds(struct partition *node, struct table_entry *table)
{
    ...
    for (i = node->top + 1; i <= node->bot; i++) {
        if (node->lr > (table + i)->r)
            node->lr = (table + i)->r;
        if (node->hr < (table + i)->r)
            node->hr = (table + i)->r;
        if (node->lg > (table + i)->g)
            node->lg = (table + i)->g;
        if (node->hg < (table + i)->g)
            node->hg = (table + i)->g;
        if (node->lb > (table + i)->b)
            node->lb = (table + i)->b;
        if (node->hb < (table + i)->b)
            node->hb = (table + i)->b;
    }
    /*
        }
...
void sort_by_node(struct partition *node, struct table_entry *table)
{
    ...
    if (rdiff >= gdiff && rdiff >= bdiff) {
        if (node->sorted != RD) {
            qsort((char *)(table + node->top), (node->bot - node->top + 1), sizeof(struct table_entry), compare_red);
            node->sorted = RD;
        }
    } else if (gdiff >= rdiff && gdiff >= bdiff) {
        if (node->sorted != GR) {
            qsort((char *)(table + node->top), (node->bot - node->top + 1), sizeof(struct table_entry), compare_green);
            node->sorted = GR;
        }
    } else {
        if (node->sorted != BL) {
            qsort((char *)(table + node->top), (node->bot - node->top + 1), sizeof(struct table_entry), compare_blue);
            node->sorted = BL;
        }
    }
}
...
int compare_freq(struct table_entry *a, struct table_entry *b) { return (b->count - a->count); }

int compare_red(const void *va, const void *vb)
{
    const struct table_entry *a = va, *b = vb;
    return (b->r - a->r);
}

int compare_green(const void *va, const void *vb)
{
    const struct table_entry *a = va, *b = vb;
    return (b->g - a->g);
}

int compare_blue(const void *va, const void *vb)
{
    const struct table_entry *a = va, *b = vb;
    return (b->b - a->b);
}
