...
RGB HSVToRGB(HSV hsv)
{
    ...
    if (hsv.s == 0.0)
        ...
        switch (i) {
        case 0:
            rgb = PctToRGB(hsv.v, t, p);
            break;
        case 1:
            rgb = PctToRGB(q, hsv.v, p);
            break;
        case 2:
            rgb = PctToRGB(p, hsv.v, t);
            break;
        case 3:
            rgb = PctToRGB(p, q, hsv.v);
            break;
        case 4:
            rgb = PctToRGB(t, p, hsv.v);
            break;
        case 5:
            rgb = PctToRGB(hsv.v, p, q);
            break;
        }
    }
    return rgb;
...
{
    return (ABS((float)(r.r - s.r)) + ABS((float)(r.g - s.g)) + ABS((float)(r.b - s.b)) / (float)MAX_INTENSITY);
}
...
int RGBToXColor(RGB r, XColor *x)
{
    x->red = r.r;
    x->green = r.g;
    x->blue = r.b;
