...
int WriteVicarHeader(FILE *f, int x, int y)
{
...
void iomheader2iheader(struct iom_iheader *iomh, struct _iheader *h)
{
...
void iheader2iomheader(struct iom_iheader *iomh, struct _iheader *h)
{
    ...
    if (iomh->eformat == iom_EDF_INVALID) {
        switch (iomh->format) {
        case iom_BYTE:
            iomh->eformat = iom_NATIVE_INT_1;
            break;
        case iom_SHORT:
            iomh->eformat = iom_NATIVE_INT_2;
            break;
        case iom_INT:
            iomh->eformat = iom_NATIVE_INT_4;
            break;
        case iom_FLOAT:
            iomh->eformat = iom_NATIVE_IEEE_REAL_4;
            break;
        case iom_DOUBLE:
            iomh->eformat = iom_NATIVE_IEEE_REAL_8;
            break;
        default:
...
FILE *LoadHeader(char *filename, struct _iheader *header)
{
    ...
    if (input == 0) {
        ...
        printf("Image is %s, %s (%s) %dx%dx%d\n", iom_Org2Str(iom_header->org), iom_Format2Str(iom_header->format),
               iom_EFormat2Str(iom_header->eformat), iom_GetSamples(iom_header->size, iom_header->org),
               iom_GetLines(iom_header->size, iom_header->org), iom_GetBands(iom_header->size, iom_header->org));
    }
...
int is_compressed(FILE *fp) { return (iom_is_compressed(fp)); }

FILE *uncompress(FILE *fp, char *fname) { return (iom_uncompress(fp, fname)); }

char *get_env_var(char *name) { return (getenv(name)); }
...
char *expand_filename(char *s) { return (iom_expand_filename(s)); }
