...
int RGB_CS(Display *display, Colormap CMap, XColor *start, XColor *end, int ncolors, XColor *colors, int rgb, int *map)
/*
...
int CreateCSData(Display *display, int ncolors, XColor *colors, int width, int height, char *CSData)
/*
 ...
 */
{
    ...
    for (i = 0; i < width; i++) {
        ...
        if (k >= ncolors)
            k = ncolors - 1;
        for (j = 0; j < height; j++) {
            pw_pack_xcolor(&colors[k], (unsigned char *)(CSData + (j * width + i) * 4));
        }
    }
    (void)display;
    return (0);
