...
extern int SetCubeImage (int i);
extern int set_plot_scale (float x1, float x2, float y1, float y2);
extern int set_cube_correction (char *buf, Button b1, Button t1, float **fptr);
extern int set_scale_factor (float f);
...
CreateRestartRequestor(Display *display, XFontStruct *font)
{
    ...
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 7, 0, 0, fg, gray, 
                                           XfTextVisual, NO_FILENAME, font, 1));
    XfAddButtonCallback(B, 0, GetSomeText, NULL);
    ...
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 7, 0, 0, fg, gray, 
                                           XfTextVisual, NO_DIRECTORY, font, 1));
    XfAddButtonCallback(B, 0, GetSomeText, NULL);
    XfActivateButton(B, ExposureMask | ButtonPressMask | KeyPressMask);
    ...
    XfAddButtonCallback(B, 0, ReadRestart, NULL);

    x += w+10;
    B = Make2State3D(display, parent, font, x, y, w, h, 1, fg, fg, bg, "SAVE");	
    XfAddButtonCallback(B, 0, SaveRestart, NULL);
    ...
    XfAddButtonCallback(B, 0, DeactivateRestartRequestor, NULL);
}

ActivateRestartRequestor(Button B, XEvent *E)
{
...
void
DeactivateRestartRequestor(Button B, XEvent *E)
{
...
write_restart(char *path)
{
    ...
    for (i=0; i < NIMAGE ; i++) {
	    ...
	    fprintf(fp,"IHEAD %d,%d,%d,%d,%d,",
                    new->iheader.dptr,
                    new->iheader.corner,
                    new->iheader.byte_order,
                    new->iheader.format,
                    new->iheader.org);
...
read_restart(Display *display, char *path)
{
    FILE *fp;
    int i;
    int j;
	...
	if((sscanf(buf,"IHEAD %d,%d,%d,%d,%d,%d,%d,"
                   "%d,%d,%d,%d,%d,%d,%d,%d,%d,"
                   "%d,%d,%d,%d,%d,%d,%d,%d,%d,"
                   "%d,%f,%f\n",
                   &new->iheader.dptr,
                   ...
                   &new->iheader.format,
                   &new->iheader.org,
                   &new->iheader.prefix[0],
                   &new->iheader.prefix[1],
                   &new->iheader.prefix[2],
                   &new->iheader.suffix[0],
                   &new->iheader.suffix[1],
                   &new->iheader.suffix[2],
                   &new->iheader.size[0],
                   &new->iheader.size[1],
                   &new->iheader.size[2],
                   &new->iheader.s_lo[0],
                   ...
                   &new->iheader.s_hi[1],
                   &new->iheader.s_hi[2],
                   &new->iheader.s_skip[0],
                   &new->iheader.s_skip[1],
                   &new->iheader.s_skip[2],
                   &new->iheader.dim[0],
                   &new->iheader.dim[1],
                   &new->iheader.dim[2],
                   &new->iheader.gain, 
                   &new->iheader.offset))==28) {
...
Apply_AutoScale(Button B, XEvent *E)
{
    char *path, *suffix;
    char **dir;
    int ndir, count, i;
