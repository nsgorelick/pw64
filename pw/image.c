...
extern int pseudocolor(unsigned char *red, unsigned char *green, unsigned char *blue, unsigned char *out, int n_red,
...
double get_data(Image new, int i)
{
    double d;
...
struct vicar_header *get_image_header(Image new)
{
    ...
    if ((fp = LoadHeader(new->filename, &iheader)) == NULL)
        return (NULL);
    fclose(fp);
...
void *get_image_data(Image new)
{
    ...
    h.s_hi[orders[h.org][1]] =
        ((new->subset.line) ? (new->subset.line + new->subset.uheight - 1) : (new->subset.line + new->subset.uheight));
    /***Modified 9/14/99**/
    ...
    if (new->iheader.format == VAX_FLOAT)
        new->iheader.format = FLOAT;
    if (new->iheader.format == VAX_INTEGER)
        new->iheader.format = INT;
    if (new->header.format == VAX_FLOAT)
        new->header.format = FLOAT;
    if (new->header.format == VAX_INTEGER)
        new->header.format = INT;
...
int stretch_color(Image new)
{
    ...
    for (i = 0; i < new->ncolors; i++) {
        out_map[i] = (unsigned char)(i + 2);
    }

    new->ncolors =
        pseudocolor((unsigned char *)rgb[0]->sdata, (unsigned char *)rgb[1]->sdata, (unsigned char *)rgb[2]->sdata,
                    (unsigned char *)new->sdata, rgb[0]->ncolors, rgb[1]->ncolors, rgb[2]->ncolors, new->band,
                    new->ncolors, (unsigned char *)lut[0], (unsigned char *)lut[1], (unsigned char *)lut[2],
                    (unsigned char *)out_map, (char)2, size);
    ...
    for (i = 0; i < nplanes; i++) {
        ...
        n = rgb[i]->ncolors - 1;
        for (j = 0; j < rgb[i]->ncolors; j++) {
            colors[i][j] = MixRGB(end[i], ((float)j / (float)n), start[i], ((float)(n - j) / (float)n));
        }
    }
    ...
    for (i = 0; i < new->ncolors; i++) {
        ...
        new->Colors[i].pixel = (unsigned long)(i + 2);
