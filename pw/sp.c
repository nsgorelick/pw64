...
void MoveScroll(Button B, XEvent *E), NewScroll(Button B, XEvent *E);
void MoveShow(Button B, XEvent *E), ToggleShow(Button B, XEvent *E);
void toggle_show();
...
int CreateShowPanel (Display *display, XFontStruct *font);
int NextRecord (int i);
int SetScroll (int i);
int ReadRecord (int i, struct filedata *d);
int PutRecord (int i, struct filedata *d, int hilite);
extern int max_rec (int fd);
int MakeOutputString (struct filedata *d, char (*buf)[256]);
int UpdateHeader (void);
int move_scroll (int off);
int FinalSelection (int i);
...
extern int DeleteWaves (void);

CreateSpecprRequestor(Display *display, XFontStruct *font)
{
    Button B;
    Window w;
 ...
 */
    ...
    B = XfCreateButton(display, ControlCase->window, 290, 0, 45, 30,
        1, BLACK(display), "PRINT", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 11, 0, 0,
        BLACK(display), WHITE(display),
        XfTextVisual, "NONE", font, 0));
    XfAddButtonCallback(B, 0, select_none, NULL);
    XfActivateButton(B, ExposureMask | ButtonPressMask);
...
ActivateSP(void)
{
    XfActivateButton(SPCase, ExposureMask | StructureNotifyMask);
}
DeactivateSP(void)
{
    XfDeactivateButton(SPCase);
}

UpdateRecords(int hilite)
{
    int i,j;
...
PutRecord(int i, struct filedata *d, int hilite)
{
...
MakeOutputString(struct filedata *d, char (*buf)[256])
{
/*
...
void
HeaderCallback(Button B, XEvent *E)
{
    UpdateHeader();
}
...
void
GetSpecprFilename(Composite C, XEvent *E)
{
...
DeactivateShowCase(void)
{
    XfDeactivateButton(ShowCase);
}

void
ToggleShowCase(Button B, XEvent *E)
{
    toggle_state(B,E);
    if (B->state) {
        ActivateShowCase();
...
CreateShowPanel(Display *display, XFontStruct *font)
{
    Button B;
    Button b;
...
void
ScrollCallback(Button B, XEvent *E)
{
    int i, j;
...
void
RequestorResizeCallback(Button B, XEvent *E)
{
    int width, height;
    Display *display;
...
specpr_text_parse(struct _tlabel *lbl, struct filedata *d)
{
...
specpr_data_parse(struct _label *lbl, struct filedata *d)
{
    char tbuf[16];
