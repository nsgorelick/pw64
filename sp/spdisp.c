...
/* and the windows will get sized */
    ...
    B = XfCreateButton(display, P->Main->window, 0, 0, 300, 50, 1, BLACK(display), "Command", 1);
    XfActivateButton(B, ExposureMask);
    B->member = (int *) P;
    P->Command = B;

    B = XfCreateButton(display, P->Main->window, 0, 0, 10, 10, 1, BLACK(display), "Axis", 1);
    ...
    B = XfCreateButton(display, P->Main->window, 0, 0, 10, 10, 1, BLACK(display), "Plot", 1);
    ...
    XfAddButtonVisual(B, 0, XfCreateVisual(B, x, y + 3, 0, 0,
                                           BLACK(display), WHITE(display), XfTextVisual, "X", font, 1));

    y += height + 5;
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 10, y + 3, 0, 0,
                                           BLACK(display), WHITE(display), XfTextVisual, "Y", font, 1));

    x = 20;
    y = 5;
    XfAddButtonVisual(B, 0, XfCreateVisual(B, x, y + 1, 40, 0,
                                           BLACK(display), WHITE(display), XfTextVisual, "Low", font, 0));

    x += width + 10;
    XfAddButtonVisual(B, 0, XfCreateVisual(B, x, y + 1, 40, 0,
                                           BLACK(display), WHITE(display), XfTextVisual, "High", font, 0));

    x = 20;
    y = 20;
    B = P->BresizeXlow = XfCreateButton(display, P->Bresize->window, x, y, width, height, 1, BLACK(display), "Xlow", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 3, 0, 0,
                                           BLACK(display), WHITE(display), XfTextVisual, "0.0", font, 0));
    ...
    x += width + 10;
    B = P->BresizeXhi = XfCreateButton(display, P->Bresize->window, x, y, width, height, 1, BLACK(display), "Xhi", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 3, 0, 0,
                                           BLACK(display), WHITE(display), XfTextVisual, "1.0", font, 0));
    XfAddButtonCallback(B, 0, getscale, NULL);
    B->member = P;
    B->ext = (char *) 2;
    XfActivateButton(B, ExposureMask | ButtonPressMask | KeyPressMask);
    ...
    x += width + 10;
    B = XfCreateButton(display, P->Bresize->window, x, y, width, height, 1, BLACK(display), "AutoX", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 3, 0, 0,
                                           BLACK(display), Background.pixel, XfTextVisual, "AUTO", font, 0));
    ...
    x = 20;
    y += height + 5;
    B = P->BresizeYlow = XfCreateButton(display, P->Bresize->window, x, y, width, height, 1, BLACK(display), "Ylow", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 3, 0, 0,
                                           BLACK(display), WHITE(display), XfTextVisual, "0.0", font, 0));
    XfAddButtonCallback(B, 0, getscale, NULL);
    B->member = (void *) P;
    ...
    x += width + 10;
    B = P->BresizeYhi = XfCreateButton(display, P->Bresize->window, x, y, width, height, 1, BLACK(display), "Yhi", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 3, 0, 0,
                                           BLACK(display), WHITE(display), XfTextVisual, "1.0", font, 0));
    XfAddButtonCallback(B, 0, getscale, NULL);
    B->member = (void *) P;
    ...
    x += width + 10;
    B = XfCreateButton(display, P->Bresize->window, x, y, width, height, 1, BLACK(display), "AutoY", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 3, 0, 0,
                                           BLACK(display), Background.pixel, XfTextVisual, "AUTO", font, 0));
    XfAddButtonCallback(B, 0, autoscale, NULL);
    B->member = (void *) P;
    ...
    B = XfCreateButton(display, P->Bresize->window, x, y, width * 3 + 20, height, 1, BLACK(display), "Done", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 3, 0, 0,
                                           BLACK(display), Background.pixel, XfTextVisual, "Done", font, 0));
...
int width, height;
{
	...
	**/
    ...
    if (width < 40 || height < 40)
        return;
    if (width != P->Main->width || height != P->Main->height || (int) P->Main->ext == -1) {
...
PlotItem *item;
{
    ...
    if (item->type == PLOT) {
        points = (XPoint *) SpToX(item->plot.x, item->plot.y,
                                  item->plot.npts, P->xlow, P->xhigh, P->ylow, P->yhigh, P->width, P->height);
        XSetForeground(P->display, P->gc, item->plot.color);
        XDrawLines(P->display, P->PlotArea->window, P->gc, points, item->plot.npts, CoordModeOrigin);
    }
}
...
XEvent *E;
{
    ...
    for (i = 0; i < 7; i++) {
        ...
        if (loc > P->yhigh || loc < P->ylow)
            continue;
        point = SpToPoint(0.0, loc, P->xlow, P->xhigh, P->ylow, P->yhigh, P->width, P->height);

        sprintf(buf, "%.4g", loc);
        XfHersheyString(B->display, B->window, gc,
                        (int) (factor * .75), point.y, fsize, fsize, 0.0, buf, ROMAN_COMPLEX, -1.0);

        /* major tic mark */
        XDrawLine(B->display, B->window, gc, (int) (0.9 * factor), point.y, (int) (factor), point.y);
        ...
        for (j = 0; j < ntics; j++) {
            ...
            if (tloc < P->ylow)
                continue;
            point = SpToPoint(0.0, tloc, P->xlow, P->xhigh, P->ylow, P->yhigh, P->width, P->height);

            XDrawLine(B->display, B->window, gc, (int) (0.95 * factor), point.y, (int) (factor), point.y);
        }
    }

    XfHersheyString(B->display, B->window, gc,
                    (int) (.1 * factor), P->height / 2, fsize, fsize, acos(-1.0) / 2, "Y AXIS", ROMAN_COMPLEX, 0.0);
    ...
    for (i = 0; i < 7; i++) {
        ...
        if (loc > P->xhigh || loc < P->xlow)
            continue;

        point = SpToPoint(loc, 0.0, P->xlow, P->xhigh, P->ylow, P->yhigh, P->width, P->height);
        sprintf(buf, "%.4g", loc);
        XfHersheyString(B->display, B->window, gc,
                        (factor - 1) + point.x, (int) (P->height + 0.3 * factor),
                        fsize, fsize, 0.0, buf, ROMAN_COMPLEX, 0.0);

        /* major tic mark */
        XDrawLine(B->display, B->window, gc,
                  (factor - 1) + point.x, (int) (P->height + 0.2 * factor), (factor - 1) + point.x, (int) (P->height));
        for (j = 0; j < ntics; j++) {
            tloc = loc - j * ticincr;
            if (tloc < P->xlow)
                continue;
            point = SpToPoint(tloc, 0.0, P->xlow, P->xhigh, P->ylow, P->yhigh, P->width, P->height);
    ...
    XfHersheyString(B->display, B->window, gc,
                    ...
                    fsize, fsize, 0.0, "X AXIS", ROMAN_COMPLEX, 0.0);
}

PlotItem **make_item(P)
Plot *P;
{
    PlotItem *item;
    PlotItem **iptr;
...
Plot *P;
{
    ...
    XFillRectangle(P->display, P->PlotArea->window, gc, 0, 0, P->PlotArea->width, P->PlotArea->height);
}
