...
char **av;
{
    ...
    if (bg == 0) {
        ...
        if (!XParseColor(display, ColorMap, "#C0C0C0", &Background) || !XAllocColor(display, ColorMap, &Background)) {
 ...
 */
    B = XfCreateButton(display,
                       RootWindow(display, DefaultScreen(display)), 300, 300, 400, 340, 1, BLACK(display), "Case", 1);
 ...
 */
    ...
    SpecprFilename = (Composite) CreateComposite(display, w, font, 15, 5, 350, 20, Background.pixel, "<>");
    ...
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 5, 0, 0,
                                           BLACK(display), WHITE(display), XfTextVisual, "Header", font, 0));
    ScrollHeader = B;
    ...
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 11, 0, 0,
                                           BLACK(display), WHITE(display), XfTextVisual, "SHOW", font, 0));
    XfAddButtonVisual(B, 1, XfCreateVisual(B, 0, 11, 0, 0,
                                           WHITE(display), BLACK(display), XfTextVisual, "SHOW", font, 0));
    XfAddButtonCallback(B, 0, ToggleShowCase, NULL);
    XfAddButtonCallback(B, 1, ToggleShowCase, NULL);
    XfActivateButton(B, ExposureMask | ButtonPressMask);

    B = XfCreateButton(display, ControlCase->window, 65, 0, 45, 30, 1, BLACK(display), "PLOT", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 1, 11, 0, 0,
                                           BLACK(display), WHITE(display), XfTextVisual, "PLOT", font, 0));
    XfAddButtonCallback(B, 0, PlotSelected, NULL);
    XfActivateButton(B, ExposureMask | ButtonPressMask);

    B = XfCreateButton(display, ControlCase->window, 120, 0, 20, 30, 1, BLACK(display), "PageUp", 2);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 0, 20, 30,
                                           BLACK(display), WHITE(display), XfPixmapVisual, 1, Bitmap_Page_Up_bits));
    XfAddButtonVisual(B, 1, XfCreateVisual(B, 0, 0, 20, 30,
                                           WHITE(display), BLACK(display), XfPixmapVisual, 1, Bitmap_Page_Up_bits));
    ...
    B = XfCreateButton(display, ControlCase->window, 145, 0, 20, 30, 1, BLACK(display), "ScrollUp", 2);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 0, 20, 30,
                                           BLACK(display), WHITE(display), XfPixmapVisual, 1, Bitmap_Scroll_Up_bits));
    XfAddButtonVisual(B, 1, XfCreateVisual(B, 0, 0, 20, 30,
                                           WHITE(display), BLACK(display), XfPixmapVisual, 1, Bitmap_Scroll_Up_bits));
    XfAddButtonCallback(B, 0, MoveScroll, NULL);
    XfAddButtonCallback(B, 1, MoveScroll, NULL);
    XfActivateButton(B, ExposureMask | ButtonPressMask | ButtonReleaseMask);
    ...
    B = XfCreateButton(display, ControlCase->window, 235, 0, 20, 30, 1, BLACK(display), "ScrollDown", 2);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 0, 20, 30,
                                           BLACK(display), WHITE(display), XfPixmapVisual, 1, Bitmap_Scroll_Down_bits));
    XfAddButtonVisual(B, 1, XfCreateVisual(B, 0, 0, 20, 30,
                                           WHITE(display), BLACK(display), XfPixmapVisual, 1, Bitmap_Scroll_Down_bits));
    XfAddButtonCallback(B, 0, MoveScroll, NULL);
    XfAddButtonCallback(B, 1, MoveScroll, NULL);
    XfActivateButton(B, ExposureMask | ButtonPressMask | ButtonReleaseMask);

    B = XfCreateButton(display, ControlCase->window, 260, 0, 20, 30, 1, BLACK(display), "PageDown", 2);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 0, 20, 30,
                                           BLACK(display), WHITE(display), XfPixmapVisual, 1, Bitmap_Page_Down_bits));
    XfAddButtonVisual(B, 1, XfCreateVisual(B, 0, 0, 20, 30,
                                           WHITE(display), BLACK(display), XfPixmapVisual, 1, Bitmap_Page_Down_bits));
    XfAddButtonCallback(B, 0, MoveScroll, NULL);
    XfAddButtonCallback(B, 1, MoveScroll, NULL);
    XfActivateButton(B, ExposureMask | ButtonPressMask | ButtonReleaseMask);

    B = XfCreateButton(display, ControlCase->window, 290, 0, 45, 30, 1, BLACK(display), "NONE", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 11, 0, 0,
                                           BLACK(display), WHITE(display), XfTextVisual, "NONE", font, 0));
    ...
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 1, 11, 0, 0,
                                           BLACK(display), WHITE(display), XfTextVisual, "CANCEL", font, 0));
...
/* NOTE: size of a record may be more than 1 line */
    ...
    if (hilite) {
        ...
        XFillRectangle(display, ScrollWindow->window, gc,
                       0, (int) (i * NLines * FontHeight), ScrollWindow->width, NLines * FontHeight);
        SelectedData = d->rec;
    }
    ...
    for (j = 0; j < NLines; j++) {
        if (buf[j][0] != '\0') {
            XDrawString(display, ScrollWindow->window,
                        ...
                        0, ((i * NLines) + (j + 1)) * FontHeight - ((FontHeight - 8) / 2), buf[j], strlen(buf[j]));
        }
    }
}
...
XFontStruct *font;
{
    ...
    B = XfCreateButton(display, RootWindow(display, DefaultScreen(display)),
                       350, 350, 101, 300, 1, BLACK(display), "SHOW", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 0, 0, 0, WHITE(display), WHITE(display), XfSolidVisual));
    ShowCase = B;
    ...
    for (i = 0; i < NITEMS; i++) {
        ...
        XfAddButtonVisual(B, 0, XfCreateVisual(B, 30, height / 2 - 3, 0, 0,
                                               BLACK(display), WHITE(display), XfTextVisual, buf, font, 1));
        XfAddButtonVisual(B, 1, XfCreateVisual(B, 30, height / 2 - 3, 0, 0,
                                               WHITE(display), BLACK(display), XfTextVisual, buf, font, 1));
        ...
        if (i == 0) {
            ...
            V[1] = XfCreateVisual(b, 2, 2, 17, 17,
                                  BLACK(display), WHITE(display), XfPixmapVisual, 1, Bitmap_CheckMark_bits);
            V[2] = XfCreateVisual(b, 2, 2, 16, 16, WHITE(display), BLACK(display), XfOutlineVisual);
            V[3] = XfCreateVisual(b, 2, 2, 17, 17,
                                  WHITE(display), BLACK(display), XfPixmapVisual, 1, Bitmap_CheckMark_bits);
        }
        for (j = 0; j < 4; j++) {
...
void RequestorResizeCallback(B, E)
Button B;
XEvent *E;
{
    int width, height;
...
XEvent *E;
{
    ...
    if (SelectedData < 0) {
        ...
        if (j <= 0) {
            ...
            P->nitems += 1;
            P->itemlist = (PlotItem **) realloc(P->itemlist, P->nitems * sizeof(PlotItem *));
...
XPoint SpToPoint(x, y, xlow, xhigh, ylow, yhigh, width, height)
float x, y;
...
XPoint *SpToX(x, y, n, xlow, xhigh, ylow, yhigh, width, height)
float *x, *y;
int n;
float xlow, xhigh, ylow, yhigh;
int width, height;
