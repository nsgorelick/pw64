...
XButton XfCreateXB(Display *disp, Window win, int x, int y, int w, int h, int hi, int lo, int fg, int bg, CallBack func,
                   int type)
{
    AButton new;
...
void XfDeactivateXB(XButton XB) { XUnmapWindow(((AButton)XB)->display, ((AButton)XB)->window); }
