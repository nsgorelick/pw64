 ...
 * Button.
 */
Button XfCreateButton(Display *display, Window parent, int x, int y, int width, int height, int border_width,
                      long unsigned int border_color, char *name, int num_states)
{
    Button new;
    Button search;
    int i;
...
Button XFCreateButton(Display *display, Window parent, int x, int y, int width, int height, int border_width,
                      long unsigned int border_color, long unsigned int bg, char *name, int num_states)
{
    Button new;
    Button search;
    int i;
...
void defaultButtonCallback(Button B, XEvent *E)
{
    ...
    while (vis != NULL) {
        ...
        case XfOutlineVisual:
            XSetForeground(B->display, localGC, vis->foreground);
            XDrawRectangle(B->display, B->window, localGC, vis->x_pos, vis->y_pos, vis->width, vis->height);
            break;
        case XfSolidVisual:
            XSetForeground(B->display, localGC, vis->foreground);
            XFillRectangle(B->display, B->window, localGC, vis->x_pos, vis->y_pos, vis->width, vis->height);
            break;
           ...
           unsigned int fg, unsigned int bg, char *name)
{
    ...
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, (h - a) / 2, 0, 0, fg, bg, XfTextVisual, name, font, 0));
    XfAddButtonVisual(B, 1, XfCreateVisual(B, 0, 0, 0, 0, fg, fg, XfSolidVisual));
    XfAddButtonVisual(B, 1, XfCreateVisual(B, 0, (h - a) / 2, 0, 0, bg, fg, XfTextVisual, name, font, 0));
             ...
             unsigned int fg, unsigned int bg, char *name)
{
    ...
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 0, 0, 0, bg, bg, XfSolidVisual));
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, (h - a) / 2, 0, 0, fg, bg, XfTextVisual, name, font, 0));
    XfAddButtonVisual(B, 0, XfCreateVisual(B, -1, -1, w, h, bc, bg, XfOutlineVisual));
    XfAddButtonVisual(B, 1, XfCreateVisual(B, 0, 0, 0, 0, fg, fg, XfSolidVisual));
    XfAddButtonVisual(B, 1, XfCreateVisual(B, 0, (h - a) / 2, 0, 0, bg, fg, XfTextVisual, name, font, 0));
    XfAddButtonCallback(B, 0, XF_CALLBACK(toggle_state), NULL);
    XfAddButtonCallback(B, 1, XF_CALLBACK(toggle_state), NULL);
...
void SetButtonState(Button B, int state)
{
    if (state < B->maxstate) {
        B->state = state;
        UpdateButton(B);
