...
int XfPushCB(CButton CB, XEvent *E)
{
    Display *display;
    XEvent Ev;
    int width, height;
...
void RedrawCB(CButton CB)
{
    ...
    if (CB->text) {
        XTextExtents(CB->font, CB->text, strlen(CB->text), &dir, &ascent, &descent, &extents);
        w = extents.width;
    }
    if (CB->pixmap) {
        w = CB->pix_w;
    ...
    XFillRectangle(disp, win, gc, 0, 0, width, CB->height);
...
Pixmap XfSetCBPixmap(CButton CB, Pixmap pixmap, int w, int h)
{
