...
/* color to hilite in */
{
    ...
    if (list->speed) {
        ...
        XFillRectangle(display, list->pixmap, list->gc, 0, 0, w, nitems * list->font_height);
...
void RefreshList(List list)
{
    ...
    XSetForeground(display, list->gc, WHITE(display));
    if (list->speed) {
        XFillRectangle(display, list->pixmap, list->gc, 0, 0, list->width, list->nitems * list->font_height);
    } else {
        XFillRectangle(display, list->view->window, list->gc, 0, 0, width, list->view->height + 1);
    }
