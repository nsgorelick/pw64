           ...
           XFontStruct *font)
{
    ...
    if (w == 0 && h == 0) {
        if (text != NULL) {
            XTextExtents(font, text, strlen(text), &dir, &ascent, &descent, &extents);
            w = extents.width;
            h = ascent + descent;
        }
    }
...
int XfPushLB(LButton LB, XEvent *E)
{
    Display *display;
    XEvent Ev;
    int width, height;
...
void RedrawLB(LButton LB)
{
    ...
    if (LB->text != NULL) {
        ...
        XTextExtents(LB->font, LB->text, strlen(LB->text), &dir, &ascent, &descent, &extents);
    ...
    if (LB->pixmap != 0) {
        ...
        XCopyPlane(display, LB->pixmap, window, gc, 0, 0, LB->pix_w, LB->pix_h, x, y, 1);
    }
    XFlush(display);
}
...
Pixmap XfSetLBPixmap(LButton LB, Pixmap pixmap, int w, int h)
{
