...
XfMenu *XfAddPeerMenu(parent, str, align, font, pixmap)
XfMenu *parent;
char *str;
int align;
XFontStruct font;
...
XfMenu *XfAddMenuItem(parent, str, align, font, pixmap, id)
XfMenu *parent;
char *str;
int align;
XFontStruct font;
...
int depth;
{
    ...
    if (parent->child)
        print_menu(parent->child, depth + 1);
    if (parent->peer)
        print_menu(parent->peer, depth);
}
