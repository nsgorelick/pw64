...
void XfActivateRBList(RButton RB, int count)
{
...
int XfPushRB(RButton RB, XEvent *E)
{
    Display *display;
    XEvent Ev;
    int width, height;
...
void RedrawRB(RButton RB)
{
    ...
    if (RB->text) {
        XTextExtents(RB->font, RB->text, strlen(RB->text), &dir, &ascent, &descent, &extents);
        w = extents.width;
    } else if (RB->pixmap) {
        w = RB->pix_w;
    }
...
RButton XfGetRB(RButton RB, int count)
{
    RButton search;
    search = RB->headRB;
