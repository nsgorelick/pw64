...
List
CreateList(Display *display, Window parent, XFontStruct *font, int x, int y, int w, int h, int sw, int fast, long unsigned int hilite, int nitems, char **items)
...
void
ReCreateList(List list, int nitems, char **items)
{
	float	thumb_scale;
	float range;
	Display *display;
...
void
ListCallback(Button B, XEvent *E)
{
...
void
AddListCallback(List list, CallBack proc)
{
	list->callback = proc;
}
...
int
ListIsDoubleClick(List list, XEvent *E)
{
	return(E->xbutton.time - list->last_time < 500 && list->last_time != 0);
}

int
draw(List list, int i, short int bg)
{
	int loc;
	Button B;
	B = list->view;
	...
	if (list->speed) {
		...
		if ((loc + list->offset) > list->height) return 0;;
		if ((loc + list->offset) < -list->font_height) return 0;;
	    XSetForeground(B->display, list->gc, bg);
