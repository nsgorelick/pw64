                     ...
                     int header, int warp, int warp_opt, ...)
{
    ...
    for (i = 0; i < nprompt; i++) {
        ...
        XfAddButtonVisual(
            Main, 0,
            XfCreateVisual(Main, 0, ypos, 0, 0, BLACK(display), WHITE(display), XfTextVisual, prompt[i], font, 0));
        ypos += 15;
    }
    ...
    xpos = 5;
    for (i = 0; i < nopts; i++) {
        Opts[i] = XFCreateButton(display, w, (xpos + step / 2 - Bwidth / 2), ypos, Bwidth, 20, 1, BLACK(display),
                                 WHITE(display), "opt", 1);
        XfAddButtonVisual(Opts[i], 0,
                          XfCreateVisual(Opts[i], 0, 0, 0, 0, WHITE(display), WHITE(display), XfSolidVisual));
        XfAddButtonVisual(
            Opts[i], 0,
            XfCreateVisual(Opts[i], 0, 7, 0, 0, BLACK(display), WHITE(display), XfTextVisual, opts[i], font, 0));
