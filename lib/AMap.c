 ...
 */
int XfAddAMapCallback(AMap A, CallBack new, CallBack old)
{
 ...
 */
int XfDelAMapCallback(AMap A, CallBack cb)
{
    struct CallBackList *loop;
    struct CallBackList *loopn;
 ...
 * width of the map
 */
int XfSetAMap(AMap A, int *data, int npoints)
{
    int i, off, base;
    ...
    if (npoints == 1) { /* special case */
        ...
        for (i = 1; i < (npoints - 1); i++)
            A->points[base].v = data[i], A->points[base].flag = 1, base += off;
 ...
 * left edge
 */
int *XfAMapValue(AMap A, int *data)
{
    int i;
...
void defaultAMapCallback(AMap A, XEvent *E)
{
    ...
    while (vis != NULL) {
        ...
        case XfSolidVisual:
            ...
            if (vis->vtype == XfSolidVisual)
                XFillRectangle(A->display, A->map, localGC, vis->x_pos, vis->y_pos, vis->width, vis->height);
            else
                XDrawRectangle(A->display, A->map, localGC, vis->x_pos, vis->y_pos, vis->width, vis->height);
            break;
        ...
        case XfOpaqueStippledVisual:
            ...
            XSetFillStyle(A->display, localGC, ((vis->vtype == XfStippledVisual) ? FillStippled : FillOpaqueStippled));
            XFillRectangle(A->display, A->map, localGC, vis->x_pos, vis->y_pos, vis->width, vis->height);
    ...
    if (A->shade_left) {
        ...
        XFillRectangle(A->display, A->map, localGC, 0, 0, A->shade_left, A->height);
        XSetFillStyle(A->display, localGC, FillSolid);
    }
    if (A->shade_right) {
        XSetForeground(A->display, localGC, A->shade_color);
    ...
    case ButtonPress:
        ...
        if (A->action_mode == XfAMapAdd) {
            if (A->points[A->left_edge + x].flag == 1)
                ...
                cur_x = x;
        } else if ((A->action_mode == XfAMapMove) || (A->action_mode == XfAMapDel)) {
 ...
 */
int XfSetAMapAction(AMap A, int action)
{
    if (A == NULL)
        return (-1);
    A->action_mode = action;
    return (0);
}
...
int XfCenterAMap(AMap A)
{
    ...
    if (data == NULL)
        return (False);
    for (i = 0; i < A->width; i++)
        data[i].v = A->points[A->left_edge + i].v, data[i].flag = A->points[A->left_edge + i].flag;
    A->left_edge = A->width;
    XfClearAMap(A);
    for (i = 0; i < A->width; i++)
        A->points[A->left_edge + i].v = data[i].v, A->points[A->left_edge + i].flag = data[i].flag;
    A->points[A->left_edge].flag = 1;
    A->points[A->left_edge + A->width - 1].flag = 1;
    free(data);
