...
int plot_mean (float start, float length, int color);
int plot_arc (float start, float length, int color, float magnitude);
...
CreateRose(Display *display, XFontStruct *font)
{
    Button B;
    Window parent;
    int i;
    ...
    B = XfCreateButton(display, parent, 100, 100, 300, 300, 
        1, WHITE(display), "Main", 1);
    ...
    x = 10;
    width = 30;
    B = XfCreateButton(display, parent, x, 40, 30, 20, 1, 
                    WHITE(display), "text", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 3, 0, 0, 
                     BLACK(display), WHITE(display), XfTextVisual, "Bins",
                     font, 1));
    ...
    B = XfCreateButton(display, parent, x, 40, width, 20, 1, 
                    BLACK(display), "nbins", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 5, 0, 0, 
                     BLACK(display), WHITE(display), XfTextVisual, "12",
                     font, 0));
    ...
    B = XfCreateButton(display, parent, x, 40, width, 10, 1, 
                    BLACK(display), "+bins", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 1, 0, 0, 
                     BLACK(display), WHITE(display), XfTextVisual, "+",
                     font, 0));
    XfAddButtonCallback(B, 0, add_bins, NULL);
    XfActivateButton(B, ExposureMask | ButtonPressMask);
    B->ext = (char *)1;

    B = XfCreateButton(display, parent, x, 50, width, 10, 1, 
                    BLACK(display), "-bins", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 3, 0, 0, 
                     BLACK(display), WHITE(display), XfTextVisual, "-",
                     font, 0));
    ...
    width = 40;
    B = XfCreateButton(display, parent, x, 40, width, 20, 1, 
                    BLACK(display), "size_bins", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 5, 0, 0, 
                     BLACK(display), pwBackground.pixel,  XfTextVisual, "15.0",
                     font, 0));
    XfActivateButton(B, ExposureMask);
    rose->SegSize = B;
    ...
    for (i = 0 ; i < 6 ; i++) {
        B = XfCreateButton(display, parent, 10, 80+i*(height+1), 
                            height, height, 1, BLACK(display), "Switch", 3);

        v[0] = XfCreateVisual(B, 2, 2, height-5, height-5, 
                              Pixels[i], WHITE(display), XfSolidVisual);

        v[1] = XfCreateVisual(B, 2, 2, height-5, height-5, Pixels[i], 
                              BLACK(display), XfSolidVisual);

        if (i == 0) {
            v[2] = XfCreateVisual(B, 2, 2, height-5, height-5, BLACK(display), 
                                  WHITE(display), XfOutlineVisual);
        }
        ...
        XfAddButtonCallback(B, 0, switches, NULL);
        XfAddButtonCallback(B, 1, switches, NULL);
        XfAddButtonCallback(B, 2, switches, NULL);
        XfActivateButton(B, ExposureMask | ButtonPressMask);
        rose->Switches[i] = B;
        B->ext = (char *)i;
    }
    ...
    B = XfCreateButton(display, parent, 60, 80, 200, 200, 
                        1, WHITE(display), "Plot", 1);
    rose->B = B;
    rose->pixmap = XCreatePixmap(display, parent, 200, 200, 8);
    Rose_draw_plot();
    /* 
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 0, 200, 200, WHITE(display), 
                      WHITE(display), XfPixmapVisual, 8, rose->pixmap));
    */
    XfAddButtonCallback(B, 0, do_plot, NULL);
    ...
    x = 10;
    width = 45;
    B = XfCreateButton(display, parent, x, 10, width, 20, 1, 
                    WHITE(display), "text", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 7, 0, 0, 
                     BLACK(display), WHITE(display), XfTextVisual, "Vectors",
                     font, 1));
    XfActivateButton(B, ExposureMask);
    ...
    width = 20;
    B = XfCreateButton(display, parent, x, 10, width, 20, 1, 
                    BLACK(display), "text", 2);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 7, 0, 0, 
                     BLACK(display), pwBackground.pixel, XfTextVisual, " ",
                     font, 0));
    XfAddButtonVisual(B, 1, XfCreateVisual(B, 0, 7, 0, 0, 
                     BLACK(display), WHITE(display), XfTextVisual, " ",
                     font, 0));
    XfAddButtonCallback(B, 0, SetRoseImage, NULL);
    XfAddButtonCallback(B, 1, SetRoseImage, NULL);
...
void
switches(Button B, XEvent *E)
{
    int i;
    switch (E->xbutton.button) {
        case Button1:
            i = GetBlockState((int)B->ext);
            if (i == 0 || i == 1) 
                i = 1-i;
            else 
                i = 1;
            set_state(B, i);
            SetBlockState((int)B->ext, i);
            if (i == 1) {
                SetCurrentBlock((int)B->ext);
            }
            break;
        case Button2:
            i = B->state;
...
Rose_draw_plot(void)
{
    ...
    /* calculate our bins */
        ...
        XSetForeground(display, gc, WHITE(display));
        XFillRectangle(display, rose->pixmap, gc, 0, 0, rose->width, rose->height);
        XSetForeground(display, gc, WHITE(display));
        XFillArc(display, rose->pixmap, gc, 10, 10, 
                 rose->width-20, rose->height-20, 0, 360*64);
        XSetForeground(display, gc, BLACK(display));

        XDrawArc(display, rose->pixmap, gc, 0, 0, 
                 rose->width-1, rose->height-1, 0, 360*64);
        XDrawArc(display, rose->pixmap, gc, 10, 10, 
                 rose->width-20, rose->height-20, 0, 360*64);
    ...
    for (i = 0 ; i < nblocks ; i++) {
        ...
        if (GetBlockState(i) != 2 && GetBlockCount(i) != 0) {
            for (n = GetFirstBlock(i) ; n != NULL ; n = n->next) {
                ...
                sp[NSeg]++;
                if (RoseImage >= 0) {
                    val = get_data(Images[RoseImage],
                                y*Images[RoseImage]->subset.width +x);
                }
                /**
                *** Screwy deleted point stuff
                **/
                if (val > 0.9e30) continue;
				v = val - radial_offset;
...
plot_mean(float start, float length, int color)
{
    int offset;
    int size;
    ...
    XFillArc(display, rose->pixmap, gc, 
        0, 0, rose->width, rose->height,
        (int)(cvt_arc(start-length)*64.0), (int)-(length*2*64.0));
    XFillArc(display, rose->pixmap, gc, 
        0, 0, rose->width, rose->height,
        (int)(cvt_arc(start+180.0-length)*64.0), (int)-(length*2*64.0));
}

plot_arc(float start, float length, int color, float magnitude)
{
    int offset;
    int size;
    ...
    XSetForeground(display, gc, Pixels[color]);
    XFillArc(display, rose->pixmap, gc, 
        10+offset, 10+offset, size - offset*2, size - offset*2, 
...
DrawBackground(void)
{
    ...
    for (i = 0 ; i < rose->nseg ; i++) {
        ...
        x2 = c*(w-10) + w;
        y2 = w - s*(w-10);
        XDrawLine(display, rose->B->window, gc, x1, y1, x2, y2);
    }
}

do_expose(void)
{
    XCopyArea(display, rose->pixmap, rose->B->window, gc, 0, 0, 
              rose->width, rose->height, 0, 0);
    DrawBackground();
    XFlush(display);
}

void
add_bins(Button B, XEvent *E)
{
    char buf[16];

	rose->nseg += (int)B->ext;
	if (rose->nseg < 2) rose->nseg = 2;
...
DeactivateRose(Display *display)
{
    XfDeactivateButton(rose->parent);
}

void
SetRoseImage(Button B, XEvent *E)
{
    int             i;
    char            buf[2];
    char            *ptr;
