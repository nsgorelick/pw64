...
void             GetScale(Button B, XEvent *E), PlotCallback(Button B, XEvent *E);
...
int fixfloat (char *str);
extern int CreateBlockID (Display *display, XFontStruct *font);
int set_plot_scale (float x1, float x2, float y1, float y2);
extern int SetPlotStruct (struct PlotStruct *plotstruct);
int do_resize (int width, int height);
extern int GetCurrentBlock (void);
extern int SetBlockState (int i, int state);
extern int SetCurrentBlock (int i);
extern int GetBlockCount (int i);
extern void *read_qube_data (int, struct _iheader *);
int SetCubeImage (int i);
extern int EnableBlock (void);
extern int GetNewText (Button B, XEvent *E, char *s, int n, char *str);
int set_scale_factor (float f);
int DrawAxis (void);
extern int GetExtendedBlock (void);
extern int GetBlockState (int i);
int zoom (int x, int y, int state);
int UpdatePlotReadout (XEvent *E);
extern int inice (float alow, float ahigh, float aamax, float *bint, float *astrt);
int CreateSpecprWritePopup (Display *display, XFontStruct *font);
int SetSpecprTitle (void);
extern struct _label *make_header (char *filename, int npixels, int waves, char *title, char *ahist, char *mhist);
extern int write_specpr (int fd, int i, struct _label *label, char *data);
...
int SetExtendedWaves (int i);
extern int ActivateBlockID (int i);
extern int GetNBlocks (void);
extern int ActivateSP (void);
extern int delete_all (int i);
int set_cube_correction (char *buf, Button b1, Button t1, float **fptr);
extern int read_specpr (int fd, int i, struct _label *label, char **data);

CreateBlockPanel(Display *display, Window parent, XFontStruct *font)
{
    Button          B;
    ...
    B = XfCreateButton(display, parent, 275, 100, 345, 280,
               1, BLACK(display), "PlotsWindow", 1);
    ...
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 0, 0, 0,
                         WHITE(display), WHITE(display),
                           XfSolidVisual));
    XfAddButtonCallback(B, 0, resize, NULL);
    PlotsWindow = B;
    ...
    for (i = 0; i < 7; i++) {
        ...
        B = XfCreateButton(display, w, x, y, width, height,
                   1, BLACK(display), "Colors", 3);
        XfAddButtonVisual(B, 0, XfCreateVisual(B, 2, 2, width - 4, height - 4,
                          Pixels[i], WHITE(display), XfSolidVisual));
        XfAddButtonVisual(B, 0, XfCreateVisual(B, 1, 1, width - 3, height - 3,
                         BLACK(display), WHITE(display), XfOutlineVisual));

        XfAddButtonVisual(B, 1, XfCreateVisual(B, 0, 0, 0, 0,
                         Pixels[i], WHITE(display), XfSolidVisual));
        XfAddButtonVisual(B, 1, XfCreateVisual(B, 1, 1, width - 3, height - 3,
                         BLACK(display), WHITE(display), XfOutlineVisual));
        XfAddButtonVisual(B, 1, XfCreateVisual(B, 0, 0, width - 1, height - 1,
                           pwHilite.pixel, WHITE(display), XfOutlineVisual));

        XfAddButtonVisual(B, 2, XfCreateVisual(B, 2, 2, width - 4, height - 4,
                         WHITE(display), WHITE(display), XfSolidVisual));
        XfAddButtonVisual(B, 2, XfCreateVisual(B, 1, 1, width - 3, height - 3,
                         BLACK(display), WHITE(display), XfOutlineVisual));

		XfAddButtonCallback(B, 0, BlockButtonCallback, NULL);
		XfAddButtonCallback(B, 1, BlockButtonCallback, NULL);
		XfAddButtonCallback(B, 2, BlockButtonCallback, NULL);
	...
	XfAddButtonCallback(B, 0, ReadYCallback, NULL);
    XfActivateButton(B, ExposureMask | ButtonPressMask);

	y += 25;
	XB = B = Make2State3D(display, w, font, x, y, 50, 20, 1, bg, bg, fg,"READ X");
	XfAddButtonCallback(B, 0, ReadXCallback, NULL);
    XfActivateButton(B, ExposureMask | ButtonPressMask);

	y += 25;
	B = Make2State3D(display, w, font, x, y, 50, 20, 1, bg, bg, fg, "LIST");
	XfAddButtonCallback(B, 0, ActivateBlockIDCallback, NULL);
    XfActivateButton(B, ExposureMask | ButtonPressMask);

	y += 25;
	B = Make2State3D(display, w, font, x, y, 50, 20, 1, bg, bg, fg, "WRITE");
	XfAddButtonCallback(B, 0, ActivateSpecprWrite, NULL);
	XfAddButtonCallback(B, 1, SpecprCancelWrite, NULL);
	B->ext = (int *)font;
    XfActivateButton(B, ExposureMask | ButtonPressMask);
	...
	XfAddButtonCallback(B, 0, Toggle_Append, NULL);
	XfAddButtonCallback(B, 1, Toggle_Append, NULL);
    XfActivateButtonState(B, 1, ExposureMask | ButtonPressMask);

	y += 25;
	B = Make2State3D(display, w, font, x, y, 50, 20, 1, bg, bg, fg, "CYCLE");
	XfAddButtonCallback(B, 0, Toggle_Cycle, NULL);
	XfAddButtonCallback(B, 1, Toggle_Cycle, NULL);
    XfActivateButton(B, ExposureMask | ButtonPressMask);
	...
	XfAddButtonCallback(B, 0, DeleteAll, NULL);
	XfAddButtonCallback(B, 0, toggle_state, NULL);
    XfActivateButton(B, ExposureMask | ButtonPressMask);
	...
	XfAddButtonCallback(B, 0, BlockAverage, NULL);
    XfActivateButton(B, ExposureMask | ButtonPressMask);
    ...
    **/
    ...
    XfAddButtonCallback(B, 0, SetCube, NULL);
    XfAddButtonCallback(B, 1, SetCube, NULL);
...
/*
    ...
    XfAddButtonVisual(B, 0, XfCreateVisual(B, -1, -1, width, height,
                         BLACK(display), WHITE(display), XfOutlineVisual));
    XfAddButtonCallback(B, 0, SetScaleFactor, NULL);
    XfActivateButton(B, ExposureMask | ButtonPressMask | KeyPressMask);
*/
	...
	x = 10;
	y += 35;
    B = XfCreateButton(display, w, x, y, 260, 34,
               0, BLACK(display), "ReadoutCase", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, x, 0, 0, 0,
                         WHITE(display), WHITE(display),
                           XfSolidVisual));
    ReadoutCase = B;

    sprintf(buf, "%s", "----");
    B = XFCreateButton(display, ReadoutCase->window, 0, 5, 40, 20,
               1, BLACK(display), pwBackground.pixel, "XPos", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 7, 0, 0,
                         BLACK(display), pwBackground.pixel,
                           XfTextVisual, buf, font, 0));
    XfActivateButton(B, ExposureMask);
    PlotXPos = B;

    sprintf(buf, "%s", "----");
    B = XFCreateButton(display, ReadoutCase->window, 50, 5, 40, 20,
               1, BLACK(display), pwBackground.pixel, "YPos", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 7, 0, 0,
                         BLACK(display), pwBackground.pixel,
                           XfTextVisual, buf, font, 0));
    XfActivateButton(B, ExposureMask);
    ...
    **/
    ...
    B = XfCreateButton(display, w, 10, 10, 10, 10,
               1, BLACK(display), " ", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 0, 0, 0,
                         WHITE(display), WHITE(display),
                           XfTextVisual, " ", font, 1));
    XfNoAutoExposeButton(B);
    XfAddButtonCallback(B, 0, AxisCallback, NULL);
    XfActivateButton(B, ExposureMask);
    AxisCase = B;
    ...
    **/
    ...
    B = XfCreateButton(display, w, 190, 5, 190, 170,
               1, BLACK(display), " ", 1);
    PlotCase = B;
    XfNoAutoExposeButton(B);
    /*
    ...
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 0, 0, 0,
                         WHITE(display), WHITE(display),
                           XfSolidVisual));
    XfAddButtonCallback(B, 0, PlotCallback, NULL);
    XfActivateButton(B, ExposureMask |
    ...
    **/
    ...
    B = XfCreateButton(display, w, 160, 240, 220, 50,
               0, BLACK(display), " ", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, x, 0, 0, 0,
                         WHITE(display), WHITE(display),
                           XfSolidVisual));
    ...
    B = XfCreateButton(display, ScaleCase->window, 0, 0, 20, 20,
               1, BLACK(display), " ", 2);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 0, 20, 20,
                         BLACK(display), WHITE(display),
                      XfPixmapVisual, 1, Bitmap_Zoom_bits));
    XfAddButtonVisual(B, 1, XfCreateVisual(B, 0, 0, 20, 20,
                         WHITE(display), BLACK(display),
                      XfPixmapVisual, 1, Bitmap_Zoom_bits));
    XfAddButtonCallback(B, 0, AutoScale, NULL);
    XfActivateButton(B, ExposureMask | ButtonPressMask);

    B = XfCreateButton(display, ScaleCase->window, 0, 25, 20, 20,
               1, BLACK(display), " ", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 1, 7, 10, 20,
                         BLACK(display), WHITE(display),
                           XfTextVisual, "U", font, 0));
    XfAddButtonCallback(B, 0, UnZoom, NULL);
    XfActivateButton(B, ExposureMask | ButtonPressMask);

    B = XfCreateButton(display, ScaleCase->window, x + 25, 0, 25, 20,
               1, BLACK(display), "X_Up", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 1, 7, 10, 20,
                         BLACK(display), WHITE(display),
                           XfTextVisual, "X", font, 0));
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 12, 2, 10, 15,
                         BLACK(display), WHITE(display),
                   XfPixmapVisual, 1, Bitmap_Arrow_Up_15_bits));
    XfAddButtonCallback(B, 0, ShiftScale, NULL);
    XfActivateButton(B, ExposureMask | ButtonPressMask);

    B = XfCreateButton(display, ScaleCase->window, x + 25, 25, 25, 20,
               1, BLACK(display), "X_Down", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 1, 7, 10, 20,
                         BLACK(display), WHITE(display),
                           XfTextVisual, "X", font, 0));
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 12, 2, 10, 15,
                         BLACK(display), WHITE(display),
                 XfPixmapVisual, 1, Bitmap_Arrow_Down_15_bits));
    XfAddButtonCallback(B, 0, ShiftScale, NULL);
    XfActivateButton(B, ExposureMask | ButtonPressMask);

    sprintf(buf, "%-.5g", PlotXMax);
    fixfloat(buf);
    B = XfCreateButton(display, ScaleCase->window, x + 55, 0, 40, 20,
               1, BLACK(display), "XMax", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 7, 0, 0,
                         BLACK(display), WHITE(display),
                           XfTextVisual, buf, font, 0));
    ...
    fixfloat(buf);
    B = XfCreateButton(display, ScaleCase->window, x + 55, 25, 40, 20,
               1, BLACK(display), "XMin", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 7, 0, 0,
                         BLACK(display), WHITE(display),
                           XfTextVisual, buf, font, 0));
    XfAddButtonCallback(B, 0, GetScale, NULL);
    ...
    **/
    ...
    B = XfCreateButton(display, ScaleCase->window, x + 25, 0, 25, 20,
               1, BLACK(display), "Y_Up", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 1, 7, 10, 20,
                         BLACK(display), WHITE(display),
                           XfTextVisual, "Y", font, 0));
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 12, 2, 10, 15,
                         BLACK(display), WHITE(display),
                   XfPixmapVisual, 1, Bitmap_Arrow_Up_15_bits));
    XfAddButtonCallback(B, 0, ShiftScale, NULL);
	...
	CreateCubeControls(B->display,
			(B->States[0]->Visuals[0]).visual.t_vis.font);
}


ActivatePlots(Display *d)
{
...
DeactivatePlots(Display *d)
{
    XfDeactivateButton(PlotsWindow);
}


void
BlockButtonCallback(Button B, XEvent *E)
{
    Button          b;
	int current;
                            ...
                            /* position to read from */
{
    ...
    if (y != lr_y) {
		...
		if (CubeImageFd < 0) CubeImageFd = open(new->filename, O_RDONLY);
...
void
SetDelFactor(Button B, XEvent *E)
{
	char buf[1024];
	int len;
	int i=0;
...
void
set_delete_points(char **V,char *TR,int Index, int Ci)
{	
...
void
SetScaleFactor(Button B, XEvent *E)
{
	char buf[256];
...
resize(Button B, XEvent *E)
{
    ...
    if (PlotsWindow->States[0]->Visuals->width == width &&
...
do_resize(int width, int height)
{
    XEvent          E;
    ...
    ScaleCase->x = 150;
    XMoveWindow(ScaleCase->display, ScaleCase->window,
            ScaleCase->x, height - 60);


    ResizeButton(AxisCase, PlotCase->x - 40, PlotCase->y,
             PlotWidth + 40, PlotHeight + 25);
    ResizeButton(PlotCase, PlotCase->x, PlotCase->y, PlotWidth, PlotHeight);

    while (XCheckWindowEvent(PlotCase->display, PlotCase->window,
                 ExposureMask, &E));
    while (XCheckWindowEvent(AxisCase->display, AxisCase->window,
                 ExposureMask, &E));
...
RedrawAll(void)
{
    ...
    XSetForeground(PlotCase->display,
         DefaultGC(PlotCase->display, DefaultScreen(PlotCase->display)),
               WHITE(PlotCase->display));
    XFillRectangle(PlotCase->display, PlotCase->window,
    DefaultGC(PlotCase->display, DefaultScreen(PlotCase->display)), 0, 0,
...
YScale(float f)
{
    ...
    d = (double) PlotHeight -
        (((double) f - (double) PlotYMin) * (double) PlotHeight /
         (double) (PlotYMax - PlotYMin));
    return (d);
}
...
XScale(float f)
{
    ...
    d = (((double) f - (double) PlotXMin) * (double) PlotWidth /
         (double) (PlotXMax - PlotXMin));
    return (d);
}
...
YUnScale(int i)
{
    return ((float) (PlotHeight - i) *
        (PlotYMax - PlotYMin) / (float) PlotHeight + PlotYMin);
}

float 
XUnScale(int i)
{
    return ((float) i * (PlotXMax - PlotXMin) / (float) PlotWidth + PlotXMin);
}

/**
 ** This routine returns MALLOC'd memory
 **/

#if 0
void
ScalePoints(block, pdata, ret, nret)
    int         block;  /* which block are we working on (color) */
	struct PointData *pdata;
    XPoint      **ret;  /* return data */
    int         *nret;  /* sizeof returned data array */
{
    XPoint         *p;
    int             i,j;
    int count= 0;
    ...
    for (j = 0; j < n; j++) {
        ...
        y = ((float)data[j]);
        if ((x <= -1.22e34 && x >= -1.24e34)||(y <= -1.22e34 && y >= -1.24e34))
			continue;

		if (SpecprOffSpectra) y = y - SpecprOffSpectra[j];
		if (SpecprMultSpectra) y = y * SpecprMultSpectra[j];
		y = y / PlotScaleFactor;
...
/**
 ** This is identical to ScalePoints, except,
 ** 	it takes float data and 
 ** 	does not divide by ScaleFactor.
 **/
PointData *
ConvertPoints(int block, PointData *pdata, float scale)
{
    XPoint         *p;
    int             i,j;
    int count= 0;
...
/*****Old way to determin Deleted points**************

        if ((x <= -1.22e34 && x >= -1.24e34) || 
			(y <= -1.22e34 && y >= -1.24e34))
			continue;
...
void
DrawPoints(int color, int n, XPoint *p)
{
...
set_plot_scale(float x1, float x2, float y1, float y2)
{
	char buf[256];
...
UnZoom(Button B, XEvent *E)
{
	...
	zl_ptr = (zl_ptr -2) % 10;
    zcount -= 2;
	set_plot_scale(zoom_list[z][0], zoom_list[z][1], 
				   zoom_list[z][2], zoom_list[z][3]);
	RedrawAll();
}

void
GetScale(Button B, XEvent *E)
{
    Display        *display;
    XFontStruct    *font;
    char            buf[256], *s;
...
void
PlotCallback(Button B, XEvent *E)
{
    switch (E->type) {
    case Expose:
        RedrawAll();
...
DrawAxis(void)
{
    ...
    for (f = y_start; f < PlotYMax; f += y_tick) {
        ...
        sprintf(buf, "%.5g", f);
        fixfloat(buf);
        XDrawLine(display, AxisCase->window,
              DefaultGC(display, DefaultScreen(display)),
              30, (int) point, 40, (int) point);

        width = XTextWidth(fs, buf, strlen(buf));

        XDrawString(display, AxisCase->window,
                DefaultGC(display, DefaultScreen(display)),
                (int) (20 - width / 2), (int) (point + fs->max_bounds.ascent + 3),
                buf, strlen(buf));
    }
    for (f = x_start; f < PlotXMax; f += x_tick) {
        ...
        XDrawLine(display, AxisCase->window,
              DefaultGC(display, DefaultScreen(display)),
              (int) point + 39, (int) PlotHeight,
              (int) point + 39, (int) (PlotHeight + 10));
        sprintf(buf, "%.5g", f);
        fixfloat(buf);
        width = XTextWidth(fs, buf, strlen(buf));
        XDrawString(display, AxisCase->window,
                DefaultGC(display, DefaultScreen(display)),
            (int) (point + 40 - width), (int) (PlotHeight + 20),
                buf, strlen(buf));
    }
    /**
    **  Minor ticks.
    **/
    ...
    for (f = x_start; f < PlotXMax; f += x_tick) {
        ...
        XDrawLine(display, AxisCase->window,
              DefaultGC(display, DefaultScreen(display)),
              (int) point + 39, (int) PlotHeight,
              (int) point + 39, (int) (PlotHeight + 5));
    }
}

void
AxisCallback(Button B, XEvent *E)
{
	(*(B->updateCallback))(B,E);
    DrawAxis();
}
void
ShiftScale(Button B, XEvent *E)
{
    float           f;
    char            buf[256];
...
void
Toggle_Rotate(Button B, XEvent *E)
{
}
void
DoCopy(Button B, XEvent *E)
{
}
void
BlockMath(Button B, XEvent *E)
{
}

void
ActivateSpecprWrite(Button B, XEvent *E)
{
...
{
    char buf[256];
    if (GetText(B, E, buf, 256, 
        strcmp(B->States[0]->Visuals->visual.t_vis.text, NO_FILENAME)) == -1) 
        return;
    SetButtonText(B, buf);
}
...
void SpecprGo(Button B, XEvent *E)
{
    ...
    if ((i = GetCurrentBlock()) != -1) {
        ...
        if (SpecprWriteType->state == 0) {
			...
			for (j = 0 ; j < npoints ; j++) {
				if (SpecprOffSpectra) avg[j] -= SpecprOffSpectra[j];
				if (SpecprMultSpectra) avg[j] *= SpecprMultSpectra[j];
				avg[j] /= scale;
			}

			sprintf(ahist, "pw:%s, %d pixels avg", 
						trim_filename(Images[CubeImage]->filename, 20), count);

			sprintf(mhist, "pw: %s, %d pixels avg", 
					trim_filename(Images[CubeImage]->filename, 20), count);

			/* there was a bug here.  EncodeBlock() didn't do anything */

            lbl = (struct _label *)make_header(".", npoints, -1, 
											   title ,ahist, mhist);
            write_specpr(fd, -1, lbl, avg);
			free(lbl);
        } else {
            for ( ; n != NULL; n = n->next) {
                sprintf(ahist, "pw: %s pixel (%d,%d)", 
                        trim_filename(Images[CubeImage]->filename, 20),
                        n->stack->y+1, n->stack->x+1);
                sprintf(mhist, "pw: %s pixel (%d,%d)", 
                        trim_filename(Images[CubeImage]->filename, 20),
                        n->stack->y+1, n->stack->x+1);
				sprintf(buf, "%s pixel (%d,%d)", title, 
                        n->stack->y+1, n->stack->x+1);
...
CreateSpecprWritePopup(Display *display, XFontStruct *font)
{
    Button B;
    ...
    B = XfCreateButton(display, parent, 100, 100, 255, 140, 1, 
                       BLACK(display), "SpecprWrite", 1);
    ...
    B = XfCreateButton(display, win, x, y, width, 20, 2, 
        BLACK(display), "filename", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 7, 0, 0,
        BLACK(display), pwBackground.pixel,
        XfTextVisual, NO_FILENAME, font, 1));
    XfAddButtonCallback(B, 0, SpecprWriteGetFilename, NULL);
    ...
    B = XfCreateButton(display, win, x, y, 40, 20, 2, 
        BLACK(display), "record", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 7, 0, 0,
        BLACK(display), pwBackground.pixel,
        XfTextVisual, "APPEND", font, 1));
    XfAddButtonCallback(B, 0, SpecprWriteGetRecord, NULL);
    XfActivateButton(B, KeyPressMask | ExposureMask | ButtonPressMask);
    SpecprWriteRecord = B;
    ...
    B = XfCreateButton(display, win, x, y, width, 20, 2, 
        BLACK(display), "title", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 7, 0, 0,
        BLACK(display), pwBackground.pixel,
        XfTextVisual, NO_TITLE, font, 1));
    XfAddButtonCallback(B, 0, SpecprWriteGetTitle, NULL);
    XfActivateButton(B, KeyPressMask | ExposureMask | ButtonPressMask);
    SpecprWriteTitle = B;
    ...
    MakeTextButton(display, win, x,y-10,
    ...
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 7, 0, 0,
        BLACK(display), WHITE(display), 
        XfTextVisual, "SCALED", font, 0));
    XfAddButtonVisual(B, 1, XfCreateVisual(B, 0, 7, 0, 0,
        BLACK(display), WHITE(display), 
        XfTextVisual, "RAW", font, 0));
    XfAddButtonCallback(B, 0, toggle_SpecprScaled , NULL);
    XfAddButtonCallback(B, 1, toggle_SpecprScaled , NULL);
    XfActivateButton(B, ExposureMask | ButtonPressMask);
    SpecprWriteScaled = B;
    ...
    B = XfCreateButton(display, win, x, y, 50, 20, 2, 
        BLACK(display), "write", 2);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 7, 0, 0,
        BLACK(display), WHITE(display), 
        XfTextVisual, "AVERAGE", font, 0));
    XfAddButtonVisual(B, 1, XfCreateVisual(B, 0, 7, 0, 0,
        BLACK(display), WHITE(display), 
        XfTextVisual, "ALL", font, 0));
    XfAddButtonCallback(B, 0, toggle_SpecprType, NULL);
    XfAddButtonCallback(B, 1, toggle_SpecprType, NULL);
    XfActivateButton(B, ExposureMask | ButtonPressMask);
    SpecprWriteType = B;

    x += width+10;

    B = XfCreateButton(display, win, x, y, 30, 20, 2, 
        BLACK(display), "go", 2);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 7, 0, 0,
        BLACK(display), WHITE(display), 
    ...
    XfAddButtonVisual(B, 1, XfCreateVisual(B, 0, 7, 0, 0,
        WHITE(display), BLACK(display), 
        XfTextVisual, "GO", font, 0));
    XfAddButtonCallback(B, 0, SpecprGo, NULL);
    XfActivateButton(B, ExposureMask | ButtonPressMask);
    SpecprWriteGo = B;
    ...
    XfAddButtonCallback(B, 0, SpecprCancelWrite, NULL);
    XfActivateButton(B, ExposureMask | ButtonPressMask);
    SpecprWriteCancel = B;
}
...
void
ApplySpecpr(int itchan, float *data)
{
    int i;
	PointData *pdata;
...
AutoScale(Button B, XEvent *E)
{
    ...
    float xlo=MAXFLOAT, xhi=-MAXFLOAT, ylo=MAXFLOAT, yhi=-MAXFLOAT;
    struct block_node *n;
    float val, *w=NULL;
    int i,j,nw;
    char buf[256];
...
void
BlockAverage(Button B, XEvent *E)
{
    struct block_node *n;
...
/***Old Get Average Method...Must be restructured because of potential deleted points*
        for ( ; n != NULL; n = n->next) {
            for (j = 0 ; j < npoints ; j++) {
                avg[j] += get_PointData(n->pdata,j)/(float)count;
            }
        }
*********************************************************************************/
		...
		for (k=0;(k<Cnt && Skip);k++){
			if (Images[CubeImage]->Dranges[k].Start <= Val && 
			   Val <= Images[CubeImage]->Dranges[k].End){
				Modifier[j]++;
				Skip=0;
			}
		}
	...
	for (j=0;j<npoints;j++){
		...
		for (j = 0 ; j < npoints ; j++) {
			if (SpecprOffSpectra) avg[j] = avg[j] - SpecprOffSpectra[j];
			if (SpecprMultSpectra) avg[j] = avg[j] *SpecprMultSpectra[j];
		}
        ...
        if (count > 2) {
		    ...
		    Skip=1;
		    for (k=0;(k<Cnt && Skip);k++){
			if (Images[CubeImage]->Dranges[k].Start <= v && 
			   v <= Images[CubeImage]->Dranges[k].End){
				Skip=0;
			}
		    }
		    if (Skip){
			if (SpecprOffSpectra) v = v -SpecprOffSpectra[j];
			if (SpecprMultSpectra) v = v *SpecprMultSpectra[j];
			v = v - avg[j];
                        sigma[j] += v*v;
		    }
                }
            ...
            for (j = 0 ; j < npoints ; j++) {
                sigma[j] = sqrt((double)(sigma[j]/(float)(count-1-Modifier[j])))/
							sqrt((double)(count-Modifier[j]));
            }
        }
...
void
toggle_SpecprType(Button B, XEvent *E)
{
	toggle_state(B,E);
	SetSpecprTitle();
}
void
toggle_SpecprScaled(Button B, XEvent *E)
{
	toggle_state(B,E);
	SetSpecprTitle();
}
...
void
ActivateBlockIDCallback(Button B, XEvent *E)
{
...
void
ReadXCallback(Button B, XEvent *E)
{
	int i;
	ApplyX = 1;
...
void
ReadYCallback(Button B, XEvent *E)
{
	ApplyX = 0;
	XSync(B->display, False);
...
void
CopyExtendedBlock(Button B, XEvent *E)
{
	struct block_node *n;
	int current;
	float *fptr;
...
char *str;
{
	...
	sprintf(buf, "sp -bg %d -q", pwBackground.pixel);
	i = GetCurrentBlock();
	if (i == -1) i = GetNBlocks();
	...
	pfp = popen(buf, "r");
	if (fgets(buf, 256, pfp) == NULL) return;
	pclose(pfp);
	...
	if (!strcmp(buf, "CANCEL\n")) {
		 ...
		 **/
		if (SP_XFilenames[i]) free(SP_XFilenames[i]);
...
Button 
CreateCubeControls(Display *display, XFontStruct *fs)
{
	Window win;
	Button B;
	...
	if (CubeControls != NULL) return(NULL);

	win = RootWindow(display, DefaultScreen(display));
    B = XfCreateButton(display, win, 275, 100, 450, 180,
               1, BLACK(display), "Cube Controls", 1);
    XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 0, 0, 0,
                         WHITE(display), WHITE(display),
                         XfSolidVisual));
	CubeControls = B;
	...
	B =  XFCreateButton(display, win, x, y, width, height, 1,
						  BLACK(display), WHITE(display), "MultFname", 2);
	XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 7, 0, 0,
            BLACK(display),pwBackground.pixel,XfTextVisual,"<NONE>",fs,1));
	XfAddButtonVisual(B, 1, XfCreateVisual(B, 0, 7, 0, 0,
            WHITE(display), BLACK(display),XfTextVisual,"working",fs,1));
	XfAddButtonCallback(B, 0, toggle_state, NULL);
	XfAddButtonCallback(B, 0, GetCubeFnames, NULL);
	XfActivateButton(B, ExposureMask | ButtonPressMask);
	Cube_MultFname = B;
	B->ext = (int)NULL;
	B->member = (char *)1;
	...
	x+= width+5;
	B =  XFCreateButton(display, win, x, y, 250, height, 1,
						  BLACK(display), WHITE(display), "MultTitle", 2);
	XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 7, 0, 0,
            BLACK(display),pwBackground.pixel,XfTextVisual,"<NONE>",fs,1));
	XfAddButtonVisual(B, 1, XfCreateVisual(B, 0, 7, 0, 0,
            WHITE(display), BLACK(display),XfTextVisual,"working",fs,1));
	...
	B =  XFCreateButton(display, win, x, y, width, height, 1,
						  BLACK(display), WHITE(display), "OffsetFname", 2);
	XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 7, 0, 0,
            BLACK(display),pwBackground.pixel,XfTextVisual,"<NONE>",fs,1));
	XfAddButtonVisual(B, 1, XfCreateVisual(B, 0, 7, 0, 0,
            WHITE(display), BLACK(display),XfTextVisual,"working",fs,1));
	...
	x+= width+5;
	B =  XFCreateButton(display, win, x, y, 250, height, 1,
						  BLACK(display), WHITE(display), "OffsetTitle", 2);
	XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 7, 0, 0,
            BLACK(display),pwBackground.pixel,XfTextVisual,"<NONE>",fs,1));
	XfAddButtonVisual(B, 1, XfCreateVisual(B, 0, 7, 0, 0,
            WHITE(display), BLACK(display),XfTextVisual,"working",fs,1));
	...
	B =  XFCreateButton(display, win, x, y, width, height, 1,
						  BLACK(display), WHITE(display), "CubeScale", 1);
	XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 7, 0, 0,
            BLACK(display),WHITE(display),XfTextVisual,"1",fs,1));
	XfAddButtonCallback(B, 0, SetScaleFactor, NULL);
	XfActivateButton(B, ExposureMask | ButtonPressMask | KeyPressMask);
	Cube_Scale = B;
	...
	B =  XFCreateButton(display, win, x, y+15, width, height, 1,
                                                  BLACK(display), WHITE(display), "CubeDelete",1);
	XfAddButtonVisual(B, 0, XfCreateVisual(B, 0, 7, 0, 0,
            BLACK(display),WHITE(display),XfTextVisual,"<NONE>",fs,1));
	XfAddButtonCallback(B, 0, SetDelFactor, NULL);
	XfActivateButton(B, ExposureMask | ButtonPressMask | KeyPressMask);
	Cube_Deleted_Points = B;



	B = Make2State3D(display, win, fs, x+60+250+25-50, y, 50, 20, 
					1, bg, bg, fg,"DONE");
	XfAddButtonCallback(B, 0, DeactivateCubeControls, NULL);
    XfActivateButton(B, ExposureMask | ButtonPressMask);
...
void
GetCubeFnames(Button B, XEvent *E)
{
	Button b1,b2,t1,t2;
	char *fname=NULL;
	char buf[256];
...
set_cube_correction(char *buf, Button b1, Button t1, float **fptr)
{
	int fd;
	char buf2[256];
	char *ptr;
	...
	if (!strncmp(buf, "NONE", 4)) {
		...
		*ptr = '\0';
		...
		if ((fd = open(buf, O_RDONLY)) < 0) {
			...
			j = read_specpr(fd, atoi(ptr), &label, fptr);
			if (j <= 0) {
				*fptr = NULL;
...
void
DeactivateCubeControls(Button B, XEvent *E)
{
	toggle_state(B,E);
...
void
SelectChanSpace(Button B, XEvent *E)
{
	ChanSpace = B->state;
	RedrawAll();
}
...
zoom(int x, int y, int state)
{
	...
	if (state == 0) {
		...
		set_plot_scale(	
						XUnScale(MIN(x, zoom_anchor_x)), 
						XUnScale(MAX(x, zoom_anchor_x)), 
						YUnScale(MAX(y, zoom_anchor_y)),
						YUnScale(MIN(y, zoom_anchor_y)));
		RedrawAll();
		zoom_anchor = 0;
	}
}
