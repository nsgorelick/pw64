...
extern int WritePopup (Display *display, Window parent, XFontStruct *font);
...
extern int ActivatePlots (Display *d);
extern int ActivateRose (Display *display);
extern int DeactivatePlots (Display *d);
extern int DeactivateRose (Display *display);
extern int LoadImageWindow (Image new);
...
quit(Button B, XEvent *E)
{
    ...
    B->ext = (char *)1;
    i = ConfirmRequestor(B->display, 
                         RootWindow(B->display, DefaultScreen(B->display)),
                         gc, font, 
                         (int)E->xbutton.x_root, (int)E->xbutton.y_root, 
                         120, 60, 
                         0, 1, 2,
                         1, "REALLY QUIT?", 2, "NO", "YES");
...
void
ActivateWrite(Button B, XEvent *E)
{
    B->state = 1;
    UpdateButton(B);

    WritePopup(B->display, RootWindow(B->display,DefaultScreen(B->display)),
         (XFontStruct *)B->ext);
...
load_image(int i)
{
    ...
    if (Images[i]->data == NULL && Images[i]->composite == 0) return;

    if ((width = Images[i]->subset.width - IWidth) < IXPos) IXPos = width;
    if ((height = Images[i]->subset.height - IHeight) < IYPos) IYPos = height;
...
load_pan(void)
{
    ...
    if (new != NULL) {
        ...
        if (t_width > x_max) t_width = x_max;
        if (t_height > y_max) t_height = y_max;
    ...
    if (CC->image != NULL && CC->image->composite == 0) {
        if (CC->image->hist_type != 2) {
            ...
            if (CC->image->hist_scale > 1) {
                ((CC->HistScale->States[0])->Visuals[0]).background = 
                    pwHilite.pixel;
            } else {
                ((CC->HistScale->States[0])->Visuals[0]).background = 
                    WHITE(CC->display);
...
load_pallate(int i)
{
    ...
    if (new->composite == 0) {
        XStoreColor(CC->display, CC->ColorMap, g1);
        XStoreColor(CC->display, CC->ColorMap, g2);
    }
    ...
    for (j = 0 ; j < NIMAGE ; j++) {
        if (new->overlays[j]) {
            ...
            RGB_CS(CC->display, CC->ColorMap, 
                &(Images[j]->C1), &(Images[j]->C2),
                Images[j]->ncolors, Colors+colors+2, 
                CC->SpreadSpace->state, Images[j]->map);
...
void
toggle_image_window(Button B, XEvent *E)
{
    ...
    if (B->state == 1) {
        ...
        XSetTransientForHint(B->display, IWindow, 
                             DefaultRootWindow(B->display));
        XFlush(B->display);
    }
}

void
toggle_requestor_window(Button B, XEvent *E)
{
    toggle_state(B,NULL);
    if (B->state == 1) {
...
gadget_buttons(Display *display, Window w, XFontStruct *font)
{
    Button b,B;
    ...
    b = XfCreateButton(display, B->window, 
        x,y,width,height,1,BLACK(display),"Image",2);
    XfAddButtonVisual(b,0,XfCreateVisual(b, 0,0,width,height,
        BLACK(display),WHITE(display),
        XfPixmapVisual, 1, map));
    XfAddButtonVisual(b,1,XfCreateVisual(b, 0,0,width,height,
        WHITE(display),BLACK(display),
        XfPixmapVisual, 1, map));
    XfAddButtonCallback(b, 0, toggle_image_window, NULL);
    XfAddButtonCallback(b, 1, toggle_image_window, NULL);
    ...
    b = XfCreateButton(display, B->window, 
        x,y,width,height,1,BLACK(display),"Magnify",2);
    XfAddButtonVisual(b,0,XfCreateVisual(b, 0,0,width,height,
        BLACK(display),WHITE(display),
        XfPixmapVisual, 1, map));
    XfAddButtonVisual(b,1,XfCreateVisual(b, 0,0,width,height,
        WHITE(display),BLACK(display),
        XfPixmapVisual, 1, map));
    XfAddButtonCallback(b, 0, toggle_mag, NULL);
    XfAddButtonCallback(b, 1, toggle_mag, NULL);
    ...
    b = XfCreateButton(display, B->window, 
        x,y,width,height,1,BLACK(display),"Plot",2);
    XfAddButtonVisual(b,0,XfCreateVisual(b, 0,0,width,height,
        BLACK(display),WHITE(display),
        XfPixmapVisual, 1, map));
    XfAddButtonVisual(b,1,XfCreateVisual(b, 0,0,width,height,
        WHITE(display),BLACK(display),
        XfPixmapVisual, 1, map));
    XfAddButtonCallback(b, 0, toggle_plot, NULL);
    XfAddButtonCallback(b, 1, toggle_plot, NULL);
    XfActivateButton(b,ExposureMask | ButtonPressMask);
    ...
    b = XfCreateButton(display, B->window, 
        x,y,width,height,1,BLACK(display),"Folder",2);
    XfAddButtonVisual(b,0,XfCreateVisual(b, 0,0,width,height,
        BLACK(display),WHITE(display),
        XfPixmapVisual, 1, map));
    XfAddButtonVisual(b,1,XfCreateVisual(b, 0,0,width,height,
        WHITE(display),BLACK(display),
        XfPixmapVisual, 1, map));
    XfAddButtonCallback(b, 0, toggle_requestor_window,NULL);
    XfAddButtonCallback(b, 1, toggle_requestor_window,NULL);
    XfActivateButton(b, ButtonPressMask | ExposureMask);
    ...
    b = XfCreateButton(display, B->window, 
        x,y,width,height,1,BLACK(display),"Write",2);
    XfAddButtonVisual(b,0,XfCreateVisual(b, 0,0,width,height,
        BLACK(display),WHITE(display),
        XfPixmapVisual, 1, map));
    XfAddButtonVisual(b,1,XfCreateVisual(b, 0,0,width,height,
        WHITE(display),BLACK(display),
        XfPixmapVisual, 1, map));
    XfAddButtonCallback(b, 0, ActivateWrite,NULL);
    XfAddButtonCallback(b, 1, ActivateWrite,NULL);
    XfActivateButton(b, ButtonPressMask | ExposureMask);
    b->ext = (char *)font;
    ...
    b = XfCreateButton(display, B->window, 
        x,y,width,height,1,BLACK(display),"Help",1);
    XfAddButtonVisual(b,0,XfCreateVisual(b, 0,0,width,height,
        BLACK(display),WHITE(display),
        XfPixmapVisual, 1, map));
    XfActivateButton(b,ExposureMask);
    ...
    b = XfCreateButton(display, B->window, 
        x,y,width,height,1,BLACK(display),"QUIT",1);
    XfAddButtonVisual(b,0,XfCreateVisual(b, 0,8,0,0,
        BLACK(display),WHITE(display),
        XfTextVisual, map, font, 0));
    XfAddButtonCallback(b, 0, quit, NULL);
    XfActivateButton(b,ExposureMask | ButtonPressMask);
    b->ext = 0;
    ...
    b = XfCreateButton(display, B->window, 
        x,y,width,height,1,BLACK(display),"Redisplay",2);
    XfAddButtonVisual(b,0,XfCreateVisual(b, 0,8,0,0,
        BLACK(display),WHITE(display),
        XfTextVisual, map, font, 0));
    XfAddButtonVisual(b,1,XfCreateVisual(b, 0,8,0,0,
        BLACK(display),pwBackground.pixel,
        XfTextVisual, "WAIT", font, 0));
    XfAddButtonCallback(b, 0, Redisplay, NULL);
    XfActivateButton(b, ExposureMask | ButtonPressMask);

}

scroll_button(Display *display, Window w)
{
    Button b;
    int x,y,width,height;
    ...
    JS = XfCreateJoystick(display, w, x, y, width, height, 1, 
                          BLACK(display), "JS1", 15, 15);
    XfAddJoystickFieldVisual(JS,
        XfCreateVisual(JS, 0, 0, 0, 0, (unsigned long)WHITE(display),
        WHITE(display), XfSolidVisual));
    XfAddJoystickThumbVisual(JS,
        XfCreateVisual(JS, 0, 0, 15, 15,
                       pwHilite.pixel, WHITE(display),
                       XfOutlineVisual));
    XfAddJoystickCallback(JS, pan_image, NULL);
    XfActivateJoystick(JS, (ExposureMask | ButtonPressMask | 
        ButtonMotionMask));
}

void
Redisplay(Button B, XEvent *E)
{
    int ncolors,j;
    ...
    for (j = 0 ; j < NIMAGE ; j++) {
        if (CC->image->overlays[j]) {
            ...
            RGB_CS(CC->display, CC->ColorMap, 
                &(Images[j]->C1), &(Images[j]->C2),
                Images[j]->ncolors, Colors+ncolors+2, 
                CC->SpreadSpace->state, Images[j]->map);
    ...
    if (Images[i]->composite == 0)  {
        ...
        for (j = 0 ; j < NIMAGE ; j++) {
            switch (Images[i]->rgb[j]) {
                ...
                case 1: AddToDnList(j, &pwRed);break;
                case 2: AddToDnList(j, &pwGreen);break;
                case 3: AddToDnList(j, &pwBlue);break;
            }
        }
    }
    ActivateDnList();
...
MaximizeImage(Display *display)
{
    ...
    if (CC->image != NULL) {
        ...
        XGetGeometry(display, IWindow, &root, &x, &y, &w, 
                &h, &bw, &depth);
