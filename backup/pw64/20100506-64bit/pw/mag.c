...
int UpdateMagnify (Display *display, struct magnify *Mag);
...
int SetMagnifyScale (struct magnify *Mag, XEvent *E);
...
ActivateMagnify(Display *display, struct magnify *Mag)
{
    XMapRaised(display,Mag->window);
    XFlush(display);
    Mag->state = 1;
}
DeactivateMagnify(Display *display, struct magnify *Mag)
{
...
SetMagnify(Display *display, Image new, struct magnify *Mag, int x, int y)
...
MagnifyCallback(struct magnify *Mag, XEvent *E)
{
    int x = 0,y = 0 ;
    switch(E->type) {
    case Expose:
...
UpdateMagnify(Display *display, struct magnify *Mag)
{
...
LocalUpdateMagnify(Display *display, struct magnify *Mag, int x1, int y1, int x2, int y2)
{
...
SetMagnifyScale(struct magnify *Mag, XEvent *E)
{
    int i;
    char b[2];
    KeySym keysym;
...
MagConvert(int xin, int yin, int *xout, int *yout)
{
...
MagUnconvert(int xin, int yin, int *xout, int *yout)
{
    extern struct magnify *Mag;
    double fact; 
...
ColorMag(int x, int y, int color, int type)
{
/*
