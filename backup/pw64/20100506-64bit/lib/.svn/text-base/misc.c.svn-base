...
int
GetText(Button B, XEvent *E, char *s, int n, int copy)
{
	char	*p, *q;
	XFontStruct *font;
...
int
ToTiff(unsigned char *s1, unsigned char *s2, int len)
{
	/**
...
int
UnTiff(unsigned char *s1, unsigned char *s2, int len)
{
	int i,j, size;
	int outp;
 ...
 **/
int
ButtonIsDown(Display *display, Window window)
{
	Window root, child;
	int root_x, root_y;
	int win_x, win_y;
