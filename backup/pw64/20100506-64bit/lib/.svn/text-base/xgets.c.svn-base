...
char	*
xgets(Display *display, Window window, int x, int y, int width, int height, long unsigned int fg, long unsigned int bg, XFontStruct *font, char *input, XEvent *e)
{
	Window w;
	int	max_width;
	int	offset;
...
int
do_keysym(Display *display, XEvent *event, char *str, int nchars)
{
    int	count;  
	char	buffer[128];
	KeySym ksym;
...
void
decode_control(Display *display, KeySym k, char *str)
{
	char	buf[8];
	int	a;
...
void
insert_str(char *s, char *t, int at)
{
	/* put t into s at 'at' */
	(void)strcat(t, s + at);
	(void)strcpy(s + at, t);
...
void
directory_requestor(Display *display, char *str)
{
...
void
file_completion(Display *display, char *str)
{
	int i;
	char buf[256];
