 ...
 *
 * $Log$
 * Revision 1.2  2001/01/16 06:04:07  gorelick
...
void XfAddAMapReadoutCallback(AMap A, CallBack new)
{
 ...
 * Create a new AMap
 */
AMap XfCreateAMap(Display *display, Window parent, int x, int y, int width, int height, int border_width, long unsigned int border_color, char *name, long unsigned int pen_color, long unsigned int shade_color, int shade_style, Pixmap shade_pattern)
{
    AMap new;
    AMap search;
 ...
 */
int XfSetAMapAction(AMap A, int mode)
{
    if (A == NULL)
        return(False);
    else
